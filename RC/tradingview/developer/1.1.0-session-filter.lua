//@version=6
strategy("PTG Smart Strategy v1.1.0 – Session Filter", overlay=true, max_bars_back=200, 
         default_qty_type=strategy.fixed, default_qty_value=1,
         commission_type=strategy.commission.percent, commission_value=0,
         calc_on_every_tick=true, initial_capital=10000)

// ===== VERSION 1.1.0 - SESSION FILTER ENHANCEMENT =====
...
londonNYOverlap = "1200-1600" // Overlap: 12:00-16:00 UTC

// Alert Settings (same as v1.0.0)
...
getSessionFilter() =>
    switch sessionMode
        "Full Time" => true
        "London Open" => time(timeframe.period, londonOpen)
        "New York Open" => time(timeframe.period, newYorkOpen)
        "London/NY Overlap" => time(timeframe.period, londonNYOverlap)
        => true

inTradingSession = getSessionFilter()

// ===== CORE LOGIC (same as v1.0.0) =====
...
if (goLong and strategy.position_size == 0)
    ...
    strategy.entry("Long", strategy.long, qty=qty, comment="PTG Long v1.1.0")
...
if (goShort and strategy.position_size == 0)
    ...
    strategy.entry("Short", strategy.short, qty=qty, comment="PTG Short v1.1.0")
...
// ===== ENHANCED ALERT SYSTEM (same as v1.0.0) =====
...
if enableAlerts and smartGoLong and canAlert("LONG")
    alert("🚀 PTG LONG v1.1.0 - " + pairSelection + " | Session:" + sessionMode + " | Entry:" + str.tostring(longEntry, "#.###") + " | SL:" + str.tostring(slLong, "#.###") + " | TP:" + str.tostring(tp1L, "#.###") + " | Risk:" + str.tostring(longRiskPips, "#.#") + "pips | WR:>50%", alert.freq_once_per_bar)
    lastAlertBar := bar_index
    lastAlertDirection := "LONG"

if enableAlerts and smartGoShort and canAlert("SHORT")
    alert("🔻 PTG SHORT v1.1.0 - " + pairSelection + " | Session:" + sessionMode + " | Entry:" + str.tostring(shortEntry, "#.###") + " | SL:" + str.tostring(slShort, "#.###") + " | TP:" + str.tostring(tp1S, "#.###") + " | Risk:" + str.tostring(shortRiskPips, "#.#") + "pips | WR:>50%", alert.freq_once_per_bar)
    lastAlertBar := bar_index
...
if enableAlerts and pushUp and alertMode == "Push Warning" and canAlert("PUSH_UP")
    alert("⚠️ PTG PUSH UP v1.1.0 - " + pairSelection + " | Session:" + sessionMode + " - Watch for TEST phase", alert.freq_once_per_bar)
    lastAlertBar := bar_index
    lastAlertDirection := "PUSH_UP"

if enableAlerts and pushDn and alertMode == "Push Warning" and canAlert("PUSH_DOWN")
    alert("⚠️ PTG PUSH DOWN v1.1.0 - " + pairSelection + " | Session:" + sessionMode + " - Watch for TEST phase", alert.freq_once_per_bar)
    lastAlertBar := bar_index
...
// ===== ENHANCED RESULTS TABLE v1.1.0 =====
var table resultsTable = table.new(position.top_right, 2, 11, bgcolor=color.white, border_width=1)
if barstate.islast
    winRate = strategy.closedtrades > 0 ? strategy.wintrades / strategy.closedtrades * 100 : 0
    profitFactor = strategy.grossprofit > 0 and strategy.grossloss > 0 ? strategy.grossprofit / strategy.grossloss : 0
    
    table.cell(resultsTable, 0, 0, "PTG v1.1.0", text_color=color.white, bgcolor=color.blue, text_size=size.small)
    table.cell(resultsTable, 1, 0, "SESSION", text_color=color.white, bgcolor=color.blue, text_size=size.small)
    ...
    table.cell(resultsTable, 0, 6, "Session Mode", text_color=color.black, text_size=size.tiny)
    sessionColor = sessionMode == "Full Time" ? color.gray : inTradingSession ? color.green : color.red
    table.cell(resultsTable, 1, 6, sessionMode, text_color=sessionColor, text_size=size.tiny)
    
    table.cell(resultsTable, 0, 7, "Session Active", text_color=color.black, text_size=size.tiny)
    table.cell(resultsTable, 1, 7, inTradingSession ? "YES" : "NO", text_color=inTradingSession ? color.green : color.red, text_size=size.tiny)
    
    table.cell(resultsTable, 0, 8, "Alerts", text_color=color.black, text_size=size.tiny)
    table.cell(resultsTable, 1, 8, enableAlerts ? "ENHANCED" : "OFF", text_color=enableAlerts ? color.green : color.red, text_size=size.tiny)
    
    table.cell(resultsTable, 0, 9, "Version", text_color=color.black, text_size=size.tiny)
    table.cell(resultsTable, 1, 9, "v1.1.0", text_color=color.purple, text_size=size.tiny)
    
    table.cell(resultsTable, 0, 10, "Last Alert", text_color=color.black, text_size=size.tiny)
    barsAgo = bar_index - lastAlertBar
    table.cell(resultsTable, 1, 10, barsAgo > 100 ? "None" : str.tostring(barsAgo) + "b ago", text_color=color.purple, text_size=size.tiny)

// Note: v1.1.0 adds session filtering to improve trade quality during high liquidity periods
