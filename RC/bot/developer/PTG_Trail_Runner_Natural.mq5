...
input group "=== RISK MANAGEMENT (AUTO LOT FOR $100 ACCOUNT) ==="
...
//=== LOT SIZE CALCULATION FOR $100 ACCOUNT ===
double CalculateOptimalLotSize(double sl_distance_pips)
{
   ...
   // So we calculate lot based on acceptable risk
   double pip_value_per_01_lot = 1.0;
   double target_lot_01_base = risk_amount / (sl_distance_pips * pip_value_per_01_lot);
   
   // Convert to actual lot size (0.01 base)
   double calculated_lot = target_lot_01_base / 10.0;
   ...
   if(EnableDebugLogs)
      Print("💰 AUTO LOT: Balance $", DoubleToString(account_balance, 0), 
