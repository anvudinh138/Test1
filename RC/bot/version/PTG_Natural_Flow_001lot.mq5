//+------------------------------------------------------------------+
//|                    PTG Natural Flow v1.5.1 - LIVE OPTIMIZED    |
//|               Tối ưu cho Live Trading với Spread Thấp          |
//|                    Đặc biệt cho Exness Pro (0.1 spread)        |
//+------------------------------------------------------------------+
#property strict
#property copyright "Copyright 2024, PTG Trading Strategy"
#property link      "https://github.com/ptg-trading"
#property version   "1.51"
#property description "PTG Natural Flow - Live Trading Optimized for Low Spread Environment"
...
input group "=== PUSH PARAMETERS (LIVE OPTIMIZED) ==="
...
input group "=== TEST PARAMETERS (LIVE OPTIMIZED) ==="
input int      TestBars           = 10;                // Allow TEST within X bars
input int      PendingTimeout     = 3;                 // Remove pendings after X bars (tighter for live)
...
// Khôi phục settings BEST version cho backtest performance
...
// Khôi phục settings BEST để maintain performance
...
input bool     UseMarketEntry     = false;             // Quay lại pending orders cho backtest
input bool     UseInstantEntry    = false;             // Theo logic gốc TEST and GO
...
input double   MaxRiskPips        = 150.0;             // No limit - let PTG breathe!
...
//=== GLOBAL VARIABLES ===
int magic_number = 77777; // Giống với BEST version
...
void TakePartialProfit()
{
   ...
   req.comment = "Live Partial TP " + DoubleToString(PartialTPPips, 1) + "p";
   req.deviation = (ulong)(MaxSlippagePips * 10); // Slippage control
   ...
   if(OrderSend(req, res))
   {
      ...
      if(EnableDebugLogs)
         Print("💰 LIVE PARTIAL TP: Closed ", DoubleToString(close_volume, 2), " lots at +", 
               DoubleToString(PartialTPPips, 1), "p | Remaining: ", DoubleToString(remaining_volume, 2));
      
      if(EnableAlerts)
         Alert("PTG LIVE 💰 Partial TP: +", DoubleToString(PartialTPPips, 1), "p");
   }
}
...
void MoveSLToBreakeven()
{
   ...
   if(OrderSend(req, res))
   {
      ...
      if(EnableDebugLogs)
         Print("🛡️ LIVE BREAKEVEN: SL moved to ", DoubleToString(be_price, (int)SymbolInfoInteger(Symbol(), SYMBOL_DIGITS)));
      
      if(EnableAlerts)
         Alert("PTG LIVE 🛡️ Breakeven activated");
   }
}
...
void TrailStopLoss(double profit_pips)
{
   ...
   // Calculate new trailing SL (tighter for live)
   ...
   if((is_long && new_sl > current_sl) || (!is_long && new_sl < current_sl))
   {
      ...
      {
         if(EnableDebugLogs)
            Print("📈 LIVE TRAIL: SL moved to ", DoubleToString(new_sl, (int)SymbolInfoInteger(Symbol(), SYMBOL_DIGITS)), 
...
bool IsPushDetected()
{
   ...
   // PUSH criteria (same as original but with live validation)
...
void CheckTestAndGo()
{
   // Traditional pending orders approach (như BEST version)
...
void ExecuteNaturalTrade(ENUM_ORDER_TYPE order_type, double entry_price)
{
   ...
   // Always use pending orders (như BEST version)
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      if(OrderGetTicket(i) && OrderGetInteger(ORDER_TICKET) == last_order_ticket)
      {
         ...
         if(bars_elapsed >= PendingTimeout)
         {
            ...
            {
               if(EnableDebugLogs)
                  Print("⏰ LIVE TIMEOUT: Removed pending order #", last_order_ticket, " after ", PendingTimeout, " bars");
...
void OnTradeTransaction(const MqlTradeTransaction& trans, const MqlTradeRequest& request, const MqlTradeResult& result)
{
   ...
   if(trans.type == TRADE_TRANSACTION_DEAL_ADD)
   {
      ...
      {
         // Entry detection - always pending orders now
         bool is_entry = (trans.deal_type == DEAL_TYPE_BUY && request.type == ORDER_TYPE_BUY_STOP) ||
