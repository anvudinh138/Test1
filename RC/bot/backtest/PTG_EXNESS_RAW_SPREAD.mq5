//+------------------------------------------------------------------+
//|                    PTG EXNESS RAW SPREAD ACCOUNT v1.0          |
//|               Realistic Backtest for Exness Raw Spread         |
//|                    Spread: 0.0-0.5p | Commission: $3.5/lot    |
//+------------------------------------------------------------------+
#property strict
#property copyright "Copyright 2024, PTG Trading Strategy"
#property version   "1.00"
#property description "PTG Natural Flow - EXNESS RAW SPREAD Account Simulation"

//=== EXNESS RAW SPREAD ACCOUNT SETTINGS ===
input group "=== EXNESS RAW SPREAD ACCOUNT CONDITIONS ==="
input double   FixedSpreadPips    = 0.5;               // Raw Spread average (0.0-1p)
input double   CommissionPerLot   = 3.5;               // $3.5 per lot round trip
input double   SlippagePips       = 0.2;               // Minimal slippage on Raw
input double   MaxSpreadPips      = 5.0;               // Don't trade if spread > 5p (realistic for Gold)
input bool     SimulateRealism    = true;              // Enable Raw Spread conditions
...
input group "=== RAW SPREAD MANAGEMENT ==="
// Optimized for Raw Spread (ultra-low spread, commission-based)
input double   BreakevenPips      = 10.0;              // +X pips => move SL to BE (account for commission)
input double   PartialTPPips      = 18.0;              // Take 30% profit at +X pips
input double   PartialTPPercent   = 30.0;              // % of position to close
input double   TrailStepPips      = 15.0;              // Trail step
input double   MinProfitPips      = 6.0;               // Min profit to keep
input int      MaxBarsInTrade     = 25;                // Time-stop

input group "=== ENTRY SETTINGS ==="
input double   EntryBufferPips    = 0.5;               // Entry buffer (tighter for Raw)
input double   SLBufferPips       = 1.5;               // SL buffer
input double   MinProfitTarget    = 12.0;              // Min profit target (account for commission)
...
input string   BotVersion         = "v1.0-ExnessRawSpread";

//=== GLOBAL VARIABLES ===
int magic_number = 22222; // Raw Spread magic
...
int OnInit()
{
   ...
   Print("🔥 PTG EXNESS RAW SPREAD ACCOUNT v1.0 STARTED!");
   Print("📊 Raw Conditions: Spread=", DoubleToString(FixedSpreadPips, 1), "p | Commission=$", DoubleToString(CommissionPerLot, 1), "/lot | Slippage=", DoubleToString(SlippagePips, 1), "p");
   Print("🎯 Raw Management: BE=", DoubleToString(BreakevenPips, 1), "p | PartialTP=", DoubleToString(PartialTPPips, 1), "p | Trail=", DoubleToString(TrailStepPips, 1), "p");
   Print("⚡ BEST FOR: High-frequency scalping with tight spreads!");
...
void ManageRawSpreadPosition()
{
   ...
   // Raw Spread: Minimal spread + commission cost
   ...
   {
      ClosePositionAtMarket("Raw Time-stop: No progress");
      return;
   }
...
void TakePartialProfit()
{
   ...
   req.comment = "Raw Partial TP " + DoubleToString(PartialTPPips, 1) + "p";
   ...
   if(OrderSend(req, res))
   {
      ...
      if(EnableDebugLogs)
         Print("🔥 RAW PARTIAL TP: ", DoubleToString(close_volume, 2), " lots at +", 
...
void MoveSLToBreakeven()
{
   ...
   if(OrderSend(req, res))
   {
      ...
      if(EnableDebugLogs)
         Print("🔥 RAW BREAKEVEN: SL moved to cover commission (", DoubleToString(cost_pips, 1), "p)");
   }
}
...
void TrailStopLoss(double net_profit_pips)
{
   ...
   if((is_long && new_sl > current_sl) || (!is_long && new_sl < current_sl))
   {
      ...
      {
         if(EnableDebugLogs)
            Print("🔥 RAW TRAIL: Net +", DoubleToString(net_profit_pips, 1), "p (after commission)");
      }
   }
}
...
void ClosePositionAtMarket(string reason)
{
   ...
   if(OrderSend(req, res))
   {
      ...
      ResetTradeState();
      if(EnableDebugLogs)
         Print("🔥 RAW CLOSED: ", reason, " | Final Commission: $", DoubleToString(final_commission, 2));
   }
}
...
double CalculateOptimalLotSize(double sl_distance_pips)
{
   ...
   // Raw Spread: Minimal spread + commission
...
void CheckTestAndGo()
{
   ...
   if(is_bullish)
      ExecuteRawSpreadTrade(ORDER_TYPE_BUY_STOP, ask + EntryBufferPips * pip_size);
   else
      ExecuteRawSpreadTrade(ORDER_TYPE_SELL_STOP, bid - EntryBufferPips * pip_size);
}

void ExecuteRawSpreadTrade(ENUM_ORDER_TYPE order_type, double entry_price)
{
   ...
   // Check profitability after commission
   double commission_pips = (CommissionPerLot * 2) / 10.0;
   double total_cost_pips = FixedSpreadPips + commission_pips + (SlippagePips * 2);
   if(sl_distance_pips < MinProfitTarget || total_cost_pips >= MinProfitTarget * 0.4)
   {
   ...
   req.tp = 0.0;
   req.magic = magic_number;
   req.comment = "Raw PTG R:" + DoubleToString(sl_distance_pips, 1) + "p C:" + DoubleToString(total_cost_pips, 1) + "p";
   ...
   if(OrderSend(req, res))
   {
      ...
      if(EnableDebugLogs)
         Print("🔥 RAW TRADE: ", (is_long ? "LONG" : "SHORT"), " ", DoubleToString(lot_size, 2), 
               " lots | Risk: ", DoubleToString(sl_distance_pips, 1), "p | Commission: $", DoubleToString(entry_commission, 2));
   }
}
   ...
   if(trans.type == TRADE_TRANSACTION_DEAL_ADD)
   {
      if(trans.deal_type == DEAL_TYPE_BUY || trans.deal_type == DEAL_TYPE_SELL)
      {
            ...
            (trans.deal_type == DEAL_TYPE_SELL && request.type == ORDER_TYPE_SELL_STOP))
         {
            ...
            if(EnableDebugLogs)
               Print("🔥 RAW ENTRY: ", (trans.deal_type == DEAL_TYPE_BUY ? "LONG" : "SHORT"), 
         ...
         else
         {
            ...
            if(EnableDebugLogs)
               Print("🔥 RAW EXIT: Gross: ", (profit_pips >= 0 ? "+" : ""), DoubleToString(profit_pips, 1), 
                     "p | Commission: -", DoubleToString(commission_pips, 1), "p | Net: ", 
...
{
   Print("🔥 PTG EXNESS RAW SPREAD ACCOUNT v1.0 STOPPED");
   Print("📊 Signals: ", signal_count, " | Trades: ", trade_count);
   Print("❌ Spread Rejects: ", rejected_by_spread, " | Commission Rejects: ", rejected_by_commission);
   Print("💸 Total Commission Paid: $", DoubleToString(total_commission_paid, 2));
   Print("⚡ RAW SPREAD SIMULATION COMPLETED!");
}
