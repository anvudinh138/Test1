//+------------------------------------------------------------------+
//|                                PTG_YOLO_BOT_M1.mq5              |
//|                                    YOLO TRADING BOT - FULL MARGIN|
//|                              1-10u Account, Burn or Earn!       |
//+------------------------------------------------------------------+
#property copyright "PTG YOLO Bot"
#property version   "1.0Y"
#property description "YOLO BOT: High frequency, 50-60% win rate, FULL MARGIN!"

//--- YOLO TRADING PARAMETERS
input group "=== PTG ORIGINAL SETTINGS ==="
input bool     UseEMAFilter       = false;             // ORIGINAL: Disable EMA filter
input bool     UseRSIFilter       = false;             // ORIGINAL: Disable RSI filter  
input int      LookbackPeriod     = 10;                // ORIGINAL: 10 period lookback

input group "=== PUSH PARAMETERS (ORIGINAL) ==="
input double   PushRangePercent   = 0.35;              // 35% - ORIGINAL SETTING
input double   ClosePercent       = 0.45;              // 45% - ORIGINAL SETTING  
input double   OppWickPercent     = 0.65;              // 65% - ORIGINAL SETTING
input double   VolHighMultiplier  = 1.0;               // 1.0x - ORIGINAL SETTING
input double   MinRangePips       = 0.0;               // NO MINIMUM - ORIGINAL

input group "=== TIME FILTER (ORIGINAL) ==="
input bool     UseTimeFilter      = false;             // ORIGINAL: Disable time filter (24/7)
input string   StartTime          = "00:00";           // ORIGINAL: Start time
input string   EndTime            = "23:59";           // ORIGINAL: End time

input group "=== TEST PARAMETERS (ORIGINAL) ==="
input int      TestBars           = 10;                // ORIGINAL: 10 bars test window
input int      PendingTimeout     = 5;                 // ORIGINAL: 5 bars timeout
input double   PullbackMax        = 0.85;              // ORIGINAL: 85% pullback
input double   VolLowMultiplier   = 2.0;               // ORIGINAL: 200% test volume

input group "=== RISK MANAGEMENT (ORIGINAL) ==="
input double   EntryBufferPips    = 0.5;               // ORIGINAL: 0.5 buffer
input double   SLBufferPips       = 0.5;               // ORIGINAL: 0.5 SL buffer
input double   TPMultiplier       = 1.5;               // ORIGINAL: 1.5x TP ratio
input bool     UseTrailingStop    = true;              // ORIGINAL: Enable trailing
input double   TrailingStopPips   = 15.0;              // ORIGINAL: 15p trailing
input double   RiskPercent        = 0.5;               // ORIGINAL: 0.5% risk (NOT YOLO!)
input double   MaxSpreadPips      = 50.0;              // ORIGINAL: 50p spread

// RSI settings removed - RSI filter disabled in original

input group "=== SYSTEM SETTINGS ==="
input bool     AllowMultiplePositions = false;        // Single position focus
input int      MinBarsBetweenTrades   = 1;             // FAST TRADING!
input bool     EnableDebugLogs    = true;              // Full logging
...
int OnInit()
{
    ...
    // RSI handle removed - not used in original
    ...
    Print("=== PTG YOLO BOT: BURN OR EARN! ===");
    Print("Symbol: ", symbol, " | Pip size: ", pip_size);
    Print("YOLO PARAMETERS:");
    Print("Push Range: ", PushRangePercent*100, "% (AGGRESSIVE!)");
    Print("Close Position: ", ClosePercent*100, "% (MORE ENTRIES!)");
    Print("Min Range: ", MinRangePips, " pips (VERY LOW!)");
    Print("Risk Percent: ", RiskPercent, "% (FULL YOLO!)");
    ...
    Print("TARGET: High frequency, 50-60% win rate, MAXIMUM ENTRIES!");
...
void OnDeinit(const int reason)
{
    ...
    double win_rate = total_trades > 0 ? (double)wins/total_trades*100 : 0;
    
    Print("=== YOLO BOT COMPLETED ===");
    ...
    Print("YOLO RESULT: ", win_rate >= 50 ? "SUCCESS! 🔥" : "BURN! 💀");
}
...
// Session function removed - using time filter instead
...
bool IsTradingAllowed()
{
    ...
    // ORIGINAL: Check time filter instead of session filter
    if(UseTimeFilter)
    {
...
//+------------------------------------------------------------------+
//| YOLO PTG Logic - HIGH FREQUENCY TRADING                         |
//+------------------------------------------------------------------+
void PTG_MainLogic()
{
    string symbol = Symbol();
    ...
    // ORIGINAL: No minimum range requirement (commented out)
    ...
    // No VWAP filter - removed for compatibility
    
    // ORIGINAL conditions (no RSI)
    ...
    if(wait_test)
    {
        ...
        if(push_bar_index >= 1 && push_bar_index <= TestBars)
        {
            ...
            bool small_range = (range <= max_range);  // ORIGINAL: no 0.8 multiplier
...
//+------------------------------------------------------------------+
//| Execute trade with YOLO risk management                         |
//+------------------------------------------------------------------+
void ExecuteTrade(ENUM_ORDER_TYPE order_type, double entry_price, double sl_price, double tp_price, string comment)
{
    ...
    lot_size = MathMin(lot_size, 1.0); // ORIGINAL: 1.0 lot safety cap
    ...
    request.magic = 77777; // ORIGINAL magic number
...
//+------------------------------------------------------------------+
//| Trade transaction handler - YOLO STYLE                         |
//+------------------------------------------------------------------+
    ...
    if(trans.type == TRADE_TRANSACTION_DEAL_ADD)
    {
        if(trans.deal_type == DEAL_TYPE_BUY || trans.deal_type == DEAL_TYPE_SELL)
        {
            if(!is_position_open)
            {
                ...
                string msg = StringFormat("🎯 YOLO ENTRY: %s %.2f lots at %.5f",
            ...
            else
            {
                ...
                bool is_win = profit_pips > 0;
                if(is_win) wins++; else losses++;
                
                double win_rate = total_trades > 0 ? (double)wins/total_trades*100 : 0;
                
                string exit_type = is_win ? "TP" : "SL";
                string profit_sign = profit_pips >= 0 ? "+" : "";
                string result_emoji = is_win ? "🎉" : "💥";
                
                string msg = StringFormat("💰 YOLO EXIT %s: %s%s%.1f pips | Entry: %.5f | Exit: %.5f | WinRate: %.1f%% (%d/%d)",
                                        exit_type, result_emoji, profit_sign, profit_pips,
                                        last_entry_price, exit_price, win_rate, wins, losses);
                Print(msg);
                if(EnableAlerts) Alert(msg);
