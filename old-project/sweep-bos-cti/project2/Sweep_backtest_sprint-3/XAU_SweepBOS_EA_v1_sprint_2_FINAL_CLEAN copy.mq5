...
// Nếu là EURUSD: đè bộ thông số mặc định hợp lý cho test nhanh
   string symbolUpper = SelectedSymbol;
   if(symbolUpper=="EURUSD")
     {
      ...
      if(!isXAU) // các cặp FX khác (generic)
        {
...
double RoundMagnet(double price)
  {
// Different RN logic per symbol type
   bool isXAU = (StringFind(SelectedSymbol,"XAU",0)>=0);
   bool isJPY = (StringFind(SelectedSymbol,"JPY",0)>=0);
   ...
   if(isXAU)
     {
      // XAU: 0.25 increments (2475.00, 2475.25, 2475.50, etc.)
      ...
      if(isJPY)
        {
         // JPY: Round to nearest 0.05 (150.00, 150.05, 150.10, etc.)
         double increment = 0.05;
         double rounded = MathRound(price / increment) * increment;
         return rounded;
        }
      else
         if(isCrypto)
           {
            // Crypto: Round to nearest 100 (65000, 65100, 65200, etc.)
           ...
           {
            // FX: Round to nearest 0.0025 (quarter pip levels)
