//+------------------------------------------------------------------+
//|                 PTG BALANCED v3.9.0 (M1 Push–Test–Go)            |
//|  - Giữ lõi v3.8.0 (wick ATR-cap, buffers/dwell động, re-arm,     |
//|    adaptive exits, RN/Spread filters)                             |
//|  - Thêm batch presets 30..39                                     |
//+------------------------------------------------------------------+
#property strict
#property version   "3.90"
#property description "PTG v3.9.0 – presets 0..39 (18/19 stable; 20..39 batch)"

#include <Trade/Trade.mqh>

//========================== INPUTS ==================================
input group "=== Preset Switch ==="
input int   InpUsecase = 18; // 0=Manual; 15..19=v3.7 family; 20..39=batch

input group "=== General ==="
input long      InpMagic                = 390001;
...
input double    InpWickFracAlt          = 0.18;      // rule B: frac>=0.18 & pips>=min(45, max(12,Strong×ATR))
...
input double    InpRoundMajorBufferPips = 6.0;       // strict default (UC18+)
input double    InpRoundMinorGridPips   = 50.0;
input double    InpRoundMinorBufferPips = 4.0;       // strict default (UC18+)
...
input double    InpMaxSpreadPips        = 12.0;      // strict default
input double    InpMaxSpreadLowVolPips  = 10.0;      // strict default
...
input double    InpBE_Pips              = 14.0;      // floors – adaptive sẽ lớn hơn nếu ATR cao
...
void ApplyUsecase(int id){
  ...
  // NEW Batch 30..39
  if(id>=30 && id<=39){
    // mặc định dựa trên UC18/19: RN/Spread strict, adaptive exits
    SetRN(true,6,4); SetSpread(12,10); SetBias(true,35,true); SetSweep(true,true);
    switch(id){
      case 30: // No-bias gating
        SetBias(false,0,true); SetPush(0.60,0.80); break;
      case 31: // Bias rất chặt, không contra + push chặt + ATRmin cao
        SetBias(true,45,false); SetPush(0.62,0.82); P.ATRMinPips=55; break;
      case 32: // RN super strict
        SetRN(true,7,5); break;
      case 33: // RN medium-light + bias mềm
        SetRN(true,4,3); SetBias(true,25,true); break;
      case 34: // Spread ultra strict (ECN)
        SetSpread(10,9); break;
      case 35: // Engine kiên nhẫn – giảm cancel
        SetEngine(14,5,60); break;
      case 36: // Engine nhanh – thị trường nhanh
        SetEngine(8,5,30); break;
      case 37: // Hard sweep (không soft fallback)
        SetSweep(true,false); break;
      case 38: // Exits bảo thủ
        SetExitFloors(16,22,24,18,14); SetSL(28,0.50); break;
      case 39: // Exits tích cực
        SetExitFloors(12,16,18,14,10); SetSL(24,0.42); break;
    }
  }
...
bool BuildPTG(int idx, Setup &s){
   ...
   double phh=0.0, pll=0.0; // hi/lo của push segment
...
int OnInit(){
   ...
   Print("PTG v3.9.0 ready. Pip=",DoubleToString(gPip,5)); return INIT_SUCCEEDED;
}
