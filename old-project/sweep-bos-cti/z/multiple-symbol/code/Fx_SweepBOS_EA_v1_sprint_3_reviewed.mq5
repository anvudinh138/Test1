            ...
            if(lots>0 && SpreadUSD()<=P.MaxSpreadUSD)
              {
               if(AllowedToOpenNow())
                 {
                  trade.Sell(lots, Sym(), 0.0, sl, 0.0);
                  g_lastOpenTime=TimeCurrent();
...
{
   // Đếm số position của CHÍNH EA hiện tại (symbol + magic)
      ...
      if(lots>0 && PositionsOnSymbol()<P.MaxOpenPositions && SpreadUSD()<=P.MaxSpreadUSD)
        {
         ...
         if(AllowedToOpenNow())
           {
            ok = trade.SellStop(lots, price, Sym(), sl, 0.0, ORDER_TIME_SPECIFIED, exp);
            g_lastOpenTime=TimeCurrent();
      ...
      if(lots>0 && PositionsOnSymbol()<P.MaxOpenPositions && SpreadUSD()<=P.MaxSpreadUSD)
        {
         ...
         if(AllowedToOpenNow())
           {
            ok = trade.BuyStop(lots, price, Sym(), sl, 0.0, ORDER_TIME_SPECIFIED, exp);
            g_lastOpenTime=TimeCurrent();
