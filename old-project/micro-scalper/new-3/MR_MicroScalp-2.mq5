...
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| (NÂNG CẤP) Hàm sửa đổi SL/TP, kiểm tra StopLevel và giá           |
//+------------------------------------------------------------------+
bool ModifyPositionSLTP(ulong ticket, double sl, double tp)
{
   // 1. Đồng bộ hóa với môi trường giao dịch để lấy giá mới nhất
   if(!TerminalInfoInteger(TERMINAL_TRADE_ALLOWED) || Bars(_Symbol, _Period) < 100) return false;
   ...
   double stop_level_price = stop_level_points * _Point;

   // 3. Kiểm tra tính hợp lệ của SL/TP so với giá thị trường HIỆN TẠI
   if(type == POSITION_TYPE_BUY)
   {
      if(sl != 0 && sl > bid - stop_level_price) { PrintFormat("Sửa lỗi SL cho lệnh BUY: SL (%.5f) quá gần giá Bid (%.5f).", sl, bid); sl = bid - stop_level_price; }
      if(tp != 0 && tp < ask + stop_level_price) { PrintFormat("Sửa lỗi TP cho lệnh BUY: TP (%.5f) quá gần giá Ask (%.5f).", tp, ask); tp = ask + stop_level_price; }
   }
   else // SELL
   {
      if(sl != 0 && sl < ask + stop_level_price) { PrintFormat("Sửa lỗi SL cho lệnh SELL: SL (%.5f) quá gần giá Ask (%.5f).", sl, ask); sl = ask + stop_level_price; }
      if(tp != 0 && tp > bid - stop_level_price) { PrintFormat("Sửa lỗi TP cho lệnh SELL: TP (%.5f) quá gần giá Bid (%.5f).", tp, bid); tp = bid - stop_level_price; }
   }

   // 4. Gửi yêu cầu sửa đổi
   ...
   {
      PrintFormat("ModifyPositionSLTP thất bại cho ticket #%d. Lỗi: %s", ticket, result.comment);
      return false;
   }
   return true;
}
...
//+------------------------------------------------------------------+
//| (NÂNG CẤP) Quản lý lệnh, tinh gọn lại logic                       |
//+------------------------------------------------------------------+
void ManageOpenTrades()
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(PositionSelectByTicket(ticket))
      {
         ...
         {
             // ... logic tính toán new_sl, new_tp giữ nguyên như cũ ...
             // (Phần này bạn không cần thay đổi)
         }
         
         // --- LOGIC TRAILING STOP ---
         if(IN_TRAIL_ENABLED && current_sl > 0)
         {
             // ... logic trailing giữ nguyên như cũ ...
         }
