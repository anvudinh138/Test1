//+------------------------------------------------------------------+
//|                                     ye_strategy_v3_6_buildfix.mq5|
//|                                  Copyright 2024, Gemini Advisor  |
//|                      Version 3.6 (Compiler Error Fixes)          |
//+------------------------------------------------------------------+
#property copyright "Copyright 2024, Gemini Advisor"
#property link      ""
#property version   "3.6"
#property description "Fixes all compiler warnings and errors. Reformats code for clarity and stability."
#property strict
...
//--- Chú ý: Tất cả giá trị dưới đây đều tính bằng Points. 1 Pip = 10 Points. ---
...
int OnInit()
{
    Print("Initializing EA v3.6 (Build Fix)...");
...
{
    // Force refresh of position pool - MT5 threading fix
    Sleep(1); // Small delay to ensure data consistency
    ...
    {
        return; // No positions to manage, only pending orders
    }
...
void ManageTrailingStopForPosition(ulong ticket)
{
    ...
    // Apply trailing stop based on user settings and position type
    if(!InpUseTrailingSL) return;
    
    // Determine trailing behavior based on position type
    bool is_entry_3 = (current_tp == 0); // Entry 3 has no TP
    ...
    if(pos_type == POSITION_TYPE_BUY)
    {
        ...
        double profit_in_points = (current_price - open_price) / _Point;
        ...
        if(current_price > open_price + start_trailing_in_points)
        {
            ...
            if(new_sl > current_sl || current_sl == 0)
            {
                ...
                {
                    Print("Trailing SL modify failed for ticket ", (string)ticket, " - Error: ", (string)GetLastError());
                }
            }
        }
    }
    else if(pos_type == POSITION_TYPE_SELL)
    {
        ...
        double profit_in_points = (open_price - current_price) / _Point;
         ...
         if(last_deal_ticket!=g_last_processed_deal_ticket)
           {
            ...
            if(HistoryDealGetInteger(last_deal_ticket,DEAL_MAGIC)==InpMagicNumber && HistoryDealGetInteger(last_deal_ticket,DEAL_ENTRY)==DEAL_ENTRY_OUT)
              {
               static string last_loss_comment="";
               string current_comment=HistoryDealGetString(last_deal_ticket,DEAL_COMMENT);
               
               // Check if this is a TP hit (profit > 0) for breakeven processing
               if(HistoryDealGetDouble(last_deal_ticket,DEAL_PROFIT) > 0 && InpMoveSLToBE_On_TP1)
               {
                  ProcessBreakevenOnTPHit(last_deal_ticket);
               }
               
               if(HistoryDealGetDouble(last_deal_ticket,DEAL_PROFIT)<0)
                 {
                  if(current_comment!=last_loss_comment)
                    {
                     g_consecutive_losses++;
                     last_loss_comment=current_comment;
...
//+------------------------------------------------------------------+
void ProcessBreakevenOnTPHit(ulong closed_deal_ticket)
{
    // Get details of the closed deal to match with related positions
    ulong closed_position_id = HistoryDealGetInteger(closed_deal_ticket, DEAL_POSITION_ID);
    double closed_entry_price = 0;
    datetime closed_open_time = 0;
    
    // Get the original position details from history
    if(HistorySelectByPosition(closed_position_id))
    {
        int total_deals = (int)HistoryDealsTotal();
        for(int d = 0; d < total_deals; d++)
        {
            ulong deal_ticket = HistoryDealGetTicket((uint)d);
            if(HistoryDealGetInteger(deal_ticket, DEAL_ENTRY) == DEAL_ENTRY_IN)
            {
                closed_entry_price = HistoryDealGetDouble(deal_ticket, DEAL_PRICE);
                closed_open_time = (datetime)HistoryDealGetInteger(deal_ticket, DEAL_TIME);
                break;
            }
        }
    }
    
    // Find positions opened at the same time (same trade set) and move their SL to breakeven
    int positions_modified = 0;
    
    for(int i = PositionsTotal() - 1; i >= 0; i--)
    {
        ulong pos_ticket = PositionGetTicket((uint)i);
        if(!PositionSelectByTicket(pos_ticket)) continue;
        if(PositionGetInteger(POSITION_MAGIC) != InpMagicNumber) continue;
        
        double pos_entry_price = PositionGetDouble(POSITION_PRICE_OPEN);
        datetime pos_open_time = (datetime)PositionGetInteger(POSITION_TIME);
        double current_tp = PositionGetDouble(POSITION_TP);
        
        // Check if this position belongs to the same trade set (same entry price and similar open time)
        bool same_entry = MathAbs(pos_entry_price - closed_entry_price) < _Point;
        bool same_time = MathAbs(pos_open_time - closed_open_time) <= 60; // Within 60 seconds
        
        if(same_entry && same_time)
        {
            // Move SL to breakeven (entry price)
            if(trade.PositionModify(pos_ticket, pos_entry_price, current_tp))
            {
                positions_modified++;
            }
        }
...
void PlaceOrderSet(ENUM_ORDER_TYPE order_type, double entry_price = 0)
{
    ...
    double stop_loss = 0; // Initialize variable
    ...
    string trade_set_comment = "JYS_v3.6_" + (string)TimeCurrent();
    int positions_to_open = InpUseMultiTP ? InpNumberOfPositions : 1;
    
    for(int i = 0; i < positions_to_open; i++)
    {
        double lot_size = CalculateLotSize(InpStopLossPoints);
        if(lot_size < SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN)) continue;
        
        double take_profit = 0;
        if(InpUseMultiTP)
        {
            if(i == 0 && InpTakeProfit1_Points > 0)
            {
                take_profit = (order_type == ORDER_TYPE_BUY || order_type == ORDER_TYPE_BUY_STOP || order_type == ORDER_TYPE_BUY_LIMIT) ? entry_price + InpTakeProfit1_Points * _Point : entry_price - InpTakeProfit1_Points * _Point;
            }
            else if(i == 1 && positions_to_open > 2 && InpTakeProfit2_Points > 0)
            {
                 take_profit = (order_type == ORDER_TYPE_BUY || order_type == ORDER_TYPE_BUY_STOP || order_type == ORDER_TYPE_BUY_LIMIT) ? entry_price + InpTakeProfit2_Points * _Point : entry_price - InpTakeProfit2_Points * _Point;
            }
        }
        
        switch(order_type)
        {
            case ORDER_TYPE_BUY:       trade.Buy(lot_size, _Symbol, entry_price, stop_loss, take_profit, trade_set_comment); break;
            case ORDER_TYPE_SELL:      trade.Sell(lot_size, _Symbol, entry_price, stop_loss, take_profit, trade_set_comment); break;
            case ORDER_TYPE_BUY_STOP:  trade.BuyStop(lot_size, entry_price, _Symbol, stop_loss, take_profit, ORDER_TIME_GTC, 0, trade_set_comment); break;
            case ORDER_TYPE_SELL_STOP: trade.SellStop(lot_size, entry_price, _Symbol, stop_loss, take_profit, ORDER_TIME_GTC, 0, trade_set_comment); break;
            case ORDER_TYPE_BUY_LIMIT: trade.BuyLimit(lot_size, entry_price, _Symbol, stop_loss, take_profit, ORDER_TIME_GTC, 0, trade_set_comment); break;
            case ORDER_TYPE_SELL_LIMIT:trade.SellLimit(lot_size, entry_price, _Symbol, stop_loss, take_profit, ORDER_TIME_GTC, 0, trade_set_comment); break;
        }
    }
}
...
{
    if(InpMoneyManagement==MM_FIXED_LOT) return InpFixedLotSize;
    
    double risk_pct=InpRiskPercent;
    double risk_amt=AccountInfoDouble(ACCOUNT_BALANCE)*(risk_pct/100.0);
    double tick_val=SymbolInfoDouble(_Symbol,SYMBOL_TRADE_TICK_VALUE);
    double tick_size=SymbolInfoDouble(_Symbol,SYMBOL_TRADE_TICK_SIZE);
    
    if(tick_val<=0||tick_size<=0||stop_loss_in_points<=0) return InpFixedLotSize;
    
    double loss_lot=(stop_loss_in_points*_Point)/tick_size*tick_val;
    if(loss_lot<=0) return InpFixedLotSize;
...
void CheckBreakoutEntry(int h1_trend)
{
    ...
    {
        double high_level=FindSwingHigh(InpBreakoutLookbackBars);
        if(high_level>0)
        {
            double entry_p = high_level + InpBreakoutOffsetPoints*_Point;
    ...
    {
        double low_level=FindSwingLow(InpBreakoutLookbackBars);
        if(low_level>0)
        {
            double entry_p = low_level - InpBreakoutOffsetPoints*_Point;
...
//+------------------------------------------------------------------+
// The rest of the functions are unchanged but reformatted for clarity
//+------------------------------------------------------------------+
void CheckForSignal()
{
    if(!CopyAllData())
    {
        g_ea_status="Error: Cannot copy data";
        return;
    }
    if(IsTradingPaused())
    {
    ...
    {
        g_ea_status="DISABLED - Outside Session";
        DeletePendingOrders();
        return;
    }
    if(PositionsTotal()>0)
    {
        g_ea_status="Position is Open";
        return;
    }
    if(IsSpreadHigh())
    {
        g_ea_status="Spread is too high";
        return;
    }
    ...
    if(InpUseDailyFilter && h1_trend!=major_trend && h1_trend!=0)
    {
        g_ea_status="DISABLED - D1/H1 Mismatch";
        DeletePendingOrders();
        return;
    }
    
    if(!IsSignalAllowed(h1_trend))
    {
        if(InpUseAdxFilter && ArraySize(h1_adx_main)>1 && h1_adx_main[1]<InpAdxThreshold)
            g_ea_status="DISABLED - Sideways (ADX)";
        else if(InpUseDiCrossover)
            g_ea_status="DISABLED - DI not crossed";
        else 
            g_ea_status="Signal Not Allowed";
    ...
    if(h1_trend==0)
    {
        g_ea_status="WAITING - No H1 Trend";
        DeletePendingOrders();
        return;
    }
    
    g_ea_status="WAITING FOR ENTRY SIGNAL...";
...
{
    if(!InpUseLossLimit) return false;
    
    if(g_trading_paused_until>0 && g_trading_paused_until<=TimeCurrent())
    {
        Print("Trading pause has ended. Resuming operation and resetting loss count.");
        g_consecutive_losses=0;
    ...
    {
        long rem_sec=g_trading_paused_until-TimeCurrent();
        g_ea_status="PAUSED - "+(string)(rem_sec/60)+" min remaining";
        return true;
    }
    
    if(g_consecutive_losses>=InpMaxConsecutiveLosses)
    {
        g_trading_paused_until=TimeCurrent()+(InpPauseDurationHours*3600);
        Print("Max consecutive losses of ",(string)InpMaxConsecutiveLosses," reached. Pausing trading for ",(string)InpPauseDurationHours," hours.");
...
bool IsSignalAllowed(int trend_direction)
{
    if(InpUseAdxFilter)
    {
        ...
        if(h1_adx_main[1] < InpAdxThreshold) return false;
    }
    
    if(InpUseDiCrossover)
    {
...
bool IsTradingSessionActive()
{
    ...
    {
        if(t.hour >= InpTradingStartHour || t.hour < InpTradingEndHour) return true;
    }
    else
    {
        if(t.hour >= InpTradingStartHour && t.hour < InpTradingEndHour) return true;
...
void OnDeinit(const int reason)
{
    ...
    Print("Deinitializing EA v3.6. Reason: ",(string)reason);
...
{
    int bh1=3;
    int bm5=InpBreakoutLookbackBars+3;
    if(CopyRates(_Symbol,InpTrendTimeframe,0,bh1,h1_rates)<bh1 || CopyRates(_Symbol,InpEntryTimeframe,0,bm5,m5_rates)<bm5) return false;
    ...
    {
        if(CopyRates(_Symbol,PERIOD_D1,0,2,d1_rates)<2 || CopyBuffer(d1_ema_handle,0,0,2,d1_ema)<2) return false;
...
void UpdateDisplayPanel()
{
    ...
    ObjectSetInteger(0,"StatusPanel_BG",OBJPROP_BORDER_TYPE,BORDER_FLAT);
    
    string pt="--- Jinguo Ye Strategy v3.6 ---\n\n";
    pt+="EA Status: "+g_ea_status+"\n\n";
    
    string d1t="N/A";
    if(InpUseDailyFilter)
    {
        int dt=GetMajorTrend();
        if(dt==1) d1t="UP";
        else if(dt==-1) d1t="DOWN";
        else d1t="Neutral";
    }
    ...
    if(InpUseAdxFilter)
    {
        if(ArraySize(h1_adx_main)>1 && h1_adx_main[1]>=InpAdxThreshold) h1r="Trending";
        else h1r="Sideways";
    }
    
    pt+="Major Trend (D1): "+d1t+"\n";
