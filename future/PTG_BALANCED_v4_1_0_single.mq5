...
input double    InpRiskPercent          = 0.5;   // mặc định 0.5%/lệnh
...
input double    InpSpreadTolPips        = 0.5;   // tolerance (allow hơi rộng hơn giới hạn)
...
input int       InpPendingExpirySec     = 180;   // 50..55 giữ pending lâu hơn
...
void ApplyUsecase(int id){
  ...
  // ===== MANUAL =====
  if(id==0){
     Print("Usecase=0 (Manual) – keep inputs as-is.");
     return;
  }


  // ===== BASE 55..64: quality-first để nâng PF =====
  SetSpread(11.5, 10.5, 0.3);          // gate chặt (nếu 0 trade hãy nới ở UC cụ thể)
  P.ATRMinPips = 50;                  // yêu cầu biến động rõ
  SetRN(true, 7.0, 5.0);              // né RN
  SetPush(0.74, 1.04);                // push mạnh/rõ
  SetWick(0.35, 0.22, 0.65, 12.0);    // wick A/B; strong = 0.65*ATR
  SetSweep(true, false);              // HARD sweep (không soft fallback)
  SetBias(true, 42.0, false);         // M5 bias dốc, không contra
  P.PendingExpiry = 150;              // treo pending vừa phải
  SetEngine(12, 5.0, 60);             // dwell/buffer/cooldown
  SetSL(22.0, 0.36);                  // SL hơi nhỏ hơn để giảm AvgLoss
  SetExitFloors(18.0, 24.0, 30.0, 18.0, 12.0); // BE/PP/TrailStart/Step/EC (trễ → nuôi win)
  P.EntryBufferPips = 5.0;            // tránh quét giả
  P.Blackout=true; P.BOstart="00:00"; P.BOend="06:55"; // chặn Asia (XAU)

  // ===== Tinh chỉnh từng UC =====
  switch(id){
    case 55: // Momentum breakout (ít lệnh, R:R cao)
      SetPush(0.78, 1.08);
      SetWick(0.36, 0.00, 0.00, 12.0);    // chỉ Rule A
      SetRN(true, 8.0, 6.0);
      SetBias(true, 45.0, false);
      SetExitFloors(18.0, 26.0, 32.0, 18.0, 12.0);
      P.EntryBufferPips = 5.5;
      break;

    case 56: // Strict PF (lọc rất chặt)
      SetSpread(9.5, 8.5, 0.1);
      P.ATRMinPips = 60;
      SetWick(0.36, 0.24, 0.70, 12.0);
      SetBias(true, 46.0, false);
      SetRN(true, 9.0, 7.0);
      SetExitFloors(18.0, 26.0, 34.0, 20.0, 12.0);
      P.EntryBufferPips = 6.0;
      break;

    case 57: // Balanced (siết vừa)
      P.ATRMinPips = 52;
      SetPush(0.74, 1.02);
      SetWick(0.35, 0.22, 0.62, 12.0);
      SetBias(true, 40.0, false);
      SetRN(true, 7.0, 5.0);
      SetExitFloors(16.0, 24.0, 30.0, 18.0, 12.0);
      P.EntryBufferPips = 5.0;
      break;

    case 58: // Contra-only when VERY strong
      P.ATRMinPips = 50;
      SetPush(0.72, 1.02);
      SetWick(0.35, 0.24, 0.70, 12.0);    // cần wick B rất mạnh
      SetBias(true, 38.0, true);          // cho contra nhưng đòi strong
      SetSweep(true, false);              // vẫn HARD
      SetRN(true, 7.0, 5.0);
      SetExitFloors(16.0, 24.0, 30.0, 18.0, 12.0);
      P.EntryBufferPips = 5.0;
      break;

    case 59: // Trend-biased + BE trễ hơn
      P.ATRMinPips = 55;
      SetPush(0.74, 1.02);
      SetWick(0.35, 0.22, 0.60, 12.0);
      SetBias(true, 44.0, false);
      SetRN(true, 8.0, 6.0);
      SetExitFloors(20.0, 26.0, 34.0, 20.0, 12.0); // nuôi win lâu hơn
      P.EntryBufferPips = 5.5;
      break;

    case 60: // Reversal (bias OFF, wick cực mạnh)
      P.ATRMinPips = 52;
      SetBias(false, 0.0, true);
      SetPush(0.70, 1.00);
      SetWick(0.36, 0.26, 0.72, 12.0);    // đảo chiều chỉ khi rât mạnh
      SetRN(true, 7.0, 5.0);
      SetExitFloors(16.0, 24.0, 30.0, 18.0, 12.0);
      P.EntryBufferPips = 5.0;
      break;

    case 61: // Bias-lite nhưng vẫn quality
      P.ATRMinPips = 50;
      SetPush(0.72, 1.02);
      SetWick(0.35, 0.22, 0.62, 12.0);
      SetBias(true, 36.0, false);
      SetRN(true, 7.0, 5.0);
      SetExitFloors(16.0, 24.0, 30.0, 18.0, 12.0);
      P.EntryBufferPips = 5.0;
      break;

    case 62: // PF booster (siết nhưng vẫn có lệnh)
      SetSpread(10.0, 9.0, 0.2);
      P.ATRMinPips = 58;
      SetPush(0.76, 1.06);
      SetWick(0.36, 0.24, 0.70, 12.0);
      SetBias(true, 46.0, false);
    ...
    case 63: // Balanced default (khuyến nghị test đầu tiên)
      SetSpread(10.5, 9.5, 0.2);
      P.ATRMinPips = 54;
      SetPush(0.74, 1.03);
      SetWick(0.35, 0.22, 0.65, 12.0);
      SetBias(true, 42.0, false);
      SetRN(true, 7.0, 5.0);
      SetExitFloors(18.0, 24.0, 32.0, 18.0, 12.0);
      P.EntryBufferPips = 5.0;
      break;

    case 64: // Breakout + retest (sweep HARD, wick mạnh)
      P.ATRMinPips = 56;
      SetPush(0.76, 1.06);
      SetWick(0.36, 0.24, 0.68, 12.0);
      SetBias(true, 44.0, false);
      SetSweep(true, false);
      SetRN(true, 8.0, 6.0);
      SetExitFloors(18.0, 26.0, 34.0, 20.0, 12.0);
  ...
  Print("Preset#",id,
    ...
    " | Wick ",DoubleToString(P.WickFracBase,2),"/",DoubleToString(P.WickFracAlt,2)," strong=",DoubleToString(P.StrongWickATR,2),
    " | Sweep ",(P.RequireSweep? "HARD":"SOFT"),
    ...
    " | Bias slope=",DoubleToString(P.M5Slope,1)," allowContra=",(P.AllowContraBiasOnStrong?"Y":"N"),
    " | BE/PP/Trail/Step/EC=",P.BE_Floor,"/",P.Partial_Floor,"/",P.TrailStart_Floor,"/",P.TrailStep_Floor,"/",P.EarlyCutFloor,
    " | EntryBuf=",DoubleToString(P.EntryBufferPips,1),
    " | Blackout=", (P.Blackout? (P.BOstart+"→"+P.BOend) : "OFF"),
    " | Lots=", DoubleToString(P.FixedLots,2)," Risk%=", DoubleToString(P.RiskPercent,2));
}
...
bool SoftSqueezeOK(double &atr_out){
   ...
   if(P.Debug && !ok) Print("BLOCK SoftSqueeze  ATR=",DoubleToString(atr_out,1),"  Min=",DoubleToString(P.ATRMinPips,1));
   return ok;
}
...
bool RoundNumberNearby(double price){
   ...
   double dMaj=MathAbs(price - MathRound(price/gMaj)*gMaj);
   double dMin=MathAbs(price - MathRound(price/gMin)*gMin);
   bool hit=(dMaj<=bMaj)||(dMin<=bMin);
   if(P.Debug && hit) Print("Round number nearby (",DoubleToString(dMaj/Pip(),1)," / ",DoubleToString(dMin/Pip(),1)," pips)");
   return hit;
}
...
bool BuildPTG(int idx, Setup &s){
   ...
   if(!pushOK){ if(P.Debug) Print("PUSH small: avg=",DoubleToString(avgP,1),"p / max=",DoubleToString(maxP,1),"p  need ",
   ...
   if(retr<P.TestRetrMinPct || retr>P.TestRetrMaxPct){ if(P.Debug) Print("TEST retr ",DoubleToString(retr,1),"% out of [",DoubleToString(P.TestRetrMinPct,1),",",DoubleToString(P.TestRetrMaxPct,1),"]"); }
   ...
   // Rule A: frac >= base & pips >= max(cap, 0.20*ATR)
   ...
   if(!(ruleA||ruleB)){ if(P.Debug) Print("Wick small: frac=",DoubleToString(100*wickFrac,1),"%, pips=",DoubleToString(wick/Pip(),1),
   ...
   if(P.RequireSweep && !swept){
      ...
      if(!(P.SweepSoftFallback && strong)){ if(P.Debug) Print("No sweep (", (isLong?"long":"short"), ")"); return false; }
   }
...
bool M5BiasFavor(const Setup &s){
   ...
   double slope=(ema[0]-ema[10])/Pip(); bool enforce=(MathAbs(slope)>=P.M5Slope);
...
double CalcPositionSizeByRisk(double sl_price){
   ...
   double dist=MathAbs(price-sl_price); if(dist<=0) return P.FixedLots;

   double tv=SymbolInfoDouble(_Symbol,SYMBOL_TRADE_TICK_VALUE);
   double ts=TickSizeVal(); if(ts<=0) ts=PointSize();
   if(tv<=0) return P.FixedLots;

   // tiền rủi ro / (tick value per 1 lot / tick size) / khoảng SL
...
double SafeVolumeForMargin(bool isLong,double desiredVol,double atPrice){
   double step = VolumeStep();
   double vol  = MathMax(desiredVol, MinLot());      // ⭐ luôn ≥ MinLot
   double needed=0.0;
   ...
   for(int i=0;i<20;i++){
      ...
      if(OrderCalcMargin(typ,_Symbol,vol,px,needed)){
         if(AccountInfoDouble(ACCOUNT_MARGIN_FREE) >= needed*1.15) break; // buffer 15%
      }
      vol = MathFloor((vol - step)/step)*step;
   }
   if(vol < MinLot()) return 0.0;     // không đủ margin cho min lot
   return vol;
}
...
bool PlacePending(bool isLong, double price, double sl){
   ...
   if(desired < minlot){
      if(P.Debug) Print("Desired lot ",DoubleToString(desired,2)," < MinLot ",DoubleToString(minlot,2)," → bump MinLot");
      desired = minlot;
   }

   double volForMargin = SafeVolumeForMargin(isLong, desired, price);
   if(volForMargin < minlot){
      if(P.Debug) Print("Skip order – not enough margin even @MinLot=",DoubleToString(minlot,2));
      return false;
   }

   // ... phần còn lại giữ nguyên (stops level, round to tick, pending send, etc.)
   // (đừng quên NormalizePrice/StopsLevel logic như bạn đang có)
   // ...
   // ví dụ:
   ...
   if(ok){
      ...
      if(P.Debug) Print("Pending ",(isLong?"BUY STOP ":"SELL STOP "),"@",DoubleToString(price,gDigits),
                        " SL=",DoubleToString(sl,gDigits)," vol=",DoubleToString(volForMargin,2),
...
void ManagePosition(){
   ...
   if(gBarsSinceEntry<=P.EarlyCutBars && pips <= -gEarlyCut_Target){
      if(P.Debug) Print("Early-cut ",DoubleToString(pips,1),"p (target ",gEarlyCut_Target,")");
   ...
   if(!gBE && pips >= gBE_Target){
      ...
      trade.PositionModify(_Symbol,NormalizePrice(sl),0.0); gBE=true; if(P.Debug) Print("Move BE");
   }
...
bool CircuitOK(double atr){
   ...
   ResetDailyIfNeeded();
   if(TimeCurrent()<gCooldownUntil){ if(P.Debug) Print("Cooldown active"); return false; }
   if(gLossCount>=P.CB_DailyStop){ if(P.Debug) Print("Daily stop reached"); return false; }
   if(gCooldownUntil>0 && TimeCurrent()>=gCooldownUntil){
      if(atr<P.CB_MinATRResume){ if(P.Debug) Print("Resume blocked by low ATR"); return false; }
      gCooldownUntil=0;
   }
   return true;
}
...
void OnNewBar(){
   if(P.Debug){
   double atr; SoftSqueezeOK(atr);                // chỉ để lấy atr
   Print("GateCheck | ATR=",DoubleToString(atr,1),
         " | Spread=",DoubleToString(SpreadPips(),1),
         " | RNavoid=", (P.RoundAvoid?"ON":"OFF"),
