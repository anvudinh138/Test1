//@version=5
strategy("PTG Smart Strategy v1.3.1 – Fix Entry Logic", overlay=true, max_bars_back=200,
...
plotshape(pushUp, title="Push Up", style=shape.triangleup, location=location.belowbar, color=color.new(color.yellow, 0), size=size.tiny, text="P↑", textcolor=color.white)
...
plotshape(testLong, title="Test Long", style=shape.circle, location=location.belowbar, color=color.new(color.lime, 0), size=size.tiny, text="T", textcolor=color.white)
...
if goLong
    ...
    strategy.entry("Long", strategy.long, qty=qty, comment="PTG Long v1.3.1")
    strategy.exit("Long Exit", "Long", limit=tp1L, stop=slLong, comment="Long Exit")

if goShort
    qty = getLotSize(shortEntry, slShort)
    // strategy.entry will automatically close opposite position
    strategy.entry("Short", strategy.short, qty=qty, comment="PTG Short v1.3.1")
...
// Store entry prices when positions open (simplified detection)
if goLong
    lastLongEntry := longEntry
if goShort
    lastShortEntry := shortEntry

// Detect exits with robust profit/loss calculation
if strategy.position_size[1] > 0 and strategy.position_size == 0
    justExitedLong := true
    exitPrice := close
    
    // Better exit reason detection using strategy data
    ...
    // Try multiple methods to get entry price and calculate P&L
    if not na(lastLongEntry)
        ...
        // Use strategy's profit data directly
        entryPriceForExit := na
        if strategy.closedtrades > 0
            lastProfitPips = strategy.closedtrades.profit(strategy.closedtrades - 1) / (isPairEURUSD ? 1.0 : 10.0)
            profitLoss := lastProfitPips
        else
            profitLoss := na
...
// Visual Signals - Compact B/S labels (all white text)
plotshape(goLong, title="BUY Signal", style=shape.labelup, location=location.belowbar, color=color.new(color.lime, 0), text="B", textcolor=color.white, size=size.small)
plotshape(goShort, title="SELL Signal", style=shape.labeldown, location=location.abovebar, color=color.new(color.red, 0), text="S", textcolor=color.white, size=size.small)

// REMOVED DEBUG MARKERS - TOO CLUTTERED

// ===== PRECISE DOT MARKERS AT EXACT EXIT PRICE =====
// Position dots exactly where price hit TP/SL
plot(justExitedLong and exitReason == "TP" ? exitPrice : na, title="Long TP Dot", style=plot.style_circles, 
     color=color.green, linewidth=4, offset=0)
plot(justExitedLong and exitReason == "SL" ? exitPrice : na, title="Long SL Dot", style=plot.style_circles, 
     color=color.red, linewidth=4, offset=0)

plot(justExitedShort and exitReason == "TP" ? exitPrice : na, title="Short TP Dot", style=plot.style_circles, 
     color=color.green, linewidth=4, offset=0)
plot(justExitedShort and exitReason == "SL" ? exitPrice : na, title="Short SL Dot", style=plot.style_circles, 
     color=color.red, linewidth=4, offset=0)

// ===== SIMPLE EXIT INFO (NO CLUTTER) =====
// Only show basic exit markers - no overwhelming labels
if justExitedLong and not na(profitLoss)
    profitSign = profitLoss >= 0 ? "+" : ""
    exitInfo = profitSign + str.tostring(profitLoss, "#.#") + "p"
    label.new(bar_index, high, exitInfo, 
              style=label.style_label_down, 
              color=profitLoss >= 0 ? color.green : color.red, 
              textcolor=color.white, 
              size=size.small)

if justExitedShort and not na(profitLoss)
    profitSign = profitLoss >= 0 ? "+" : ""
    exitInfo = profitSign + str.tostring(profitLoss, "#.#") + "p"
    label.new(bar_index, low, exitInfo, 
              style=label.style_label_up, 
              color=profitLoss >= 0 ? color.green : color.red, 
              textcolor=color.white, 
              size=size.small)

// ===== ULTRA CLEAN SL/TP LINES =====
// Only show SL/TP during active positions, hide immediately when position closes
showSLTP = strategy.position_size != 0

plot(showSLTP and strategy.position_size > 0 ? slLong : na, title="SL Long", style=plot.style_linebr, color=color.red, linewidth=1)
plot(showSLTP and strategy.position_size < 0 ? slShort : na, title="SL Short", style=plot.style_linebr, color=color.red, linewidth=1)
plot(showSLTP and strategy.position_size > 0 ? tp1L : na, title="TP Long", style=plot.style_linebr, color=color.green, linewidth=1)
plot(showSLTP and strategy.position_size < 0 ? tp1S : na, title="TP Short", style=plot.style_linebr, color=color.green, linewidth=1)
...
if enableAlerts and goLong and canAlert("LONG")
    ...
    alert("🚀 PTG LONG - " + pairSelection + " | Entry:" + str.tostring(longEntry, "#.###") + " | SL:" + str.tostring(slLong, "#.###") + " | TP:" + str.tostring(tp1L, "#.###") + " | Risk:" + str.tostring(longRiskPips, "#.#") + "pips" + sessionInfo, alert.freq_once_per_bar)
    lastAlertBar := bar_index
    lastAlertDirection := "LONG"

if enableAlerts and goShort and canAlert("SHORT")
    sessionInfo = sessionMode != "Full Time" ? " | Session: " + sessionMode : ""
    alert("🔻 PTG SHORT - " + pairSelection + " | Entry:" + str.tostring(shortEntry, "#.###") + " | SL:" + str.tostring(slShort, "#.###") + " | TP:" + str.tostring(tp1S, "#.###") + " | Risk:" + str.tostring(shortRiskPips, "#.#") + "pips" + sessionInfo, alert.freq_once_per_bar)
    lastAlertBar := bar_index
...
if barstate.islast
    ...
    table.cell(resultsTable, 0, 0, "PTG v1.3.1", text_color=color.white, bgcolor=color.blue, text_size=size.small)
    table.cell(resultsTable, 1, 0, "ORIGINAL TP", text_color=color.white, bgcolor=color.blue, text_size=size.small)
    ...
    table.cell(resultsTable, 0, 8, "Session", text_color=color.black, text_size=size.tiny)
    table.cell(resultsTable, 1, 8, sessionMode, text_color=color.purple, text_size=size.tiny)
    
    table.cell(resultsTable, 0, 9, "In Session", text_color=color.black, text_size=size.tiny)
    table.cell(resultsTable, 1, 9, inTradingSession ? "✅ Active" : "❌ Inactive", text_color=inTradingSession ? color.green : color.red, text_size=size.tiny)
...
// Note: v1.3.1 reverts to v1.0.1 logic but with session filter
