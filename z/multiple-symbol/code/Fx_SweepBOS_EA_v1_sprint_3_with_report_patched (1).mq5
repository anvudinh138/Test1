...
// Run many symbols from ONE chart via OnTimer (supports sub-second timer)
input bool   UseMultiSymbol   = true;
input string InpSymbolsCSV    = "XAUUSD,EURUSD,GBPUSD,USDJPY";
input bool   UseMsTimer       = true;     // Use EventSetMillisecondTimer for <1s
input int    InpTimerMs       = 200;      // 200 ms (0.2s)
input int    InpTimerSeconds  = 1;        // Fallback seconds timer
input int    MagicBase        = 532100;   // Magic per symbol: MagicBase + idx

// Fixed-lot safety by default (override risk model)
...
bool SendPending(bool isShort, double price, double lots, double sl, double tp, datetime exp);

// --- Per-symbol state pack (mirror key globals) ---
struct SymState {
   string    sym;
...
// Trả về index của symbol trong gSyms; không thấy thì trả 0 (fallback an toàn)
int FindSymIdx(const string s)
{
...
double SymbolPoint() { return SymbolInfoDouble(Sym(), SYMBOL_POINT); }
   ...
   if(atr_handle==INVALID_HANDLE || last_period!=P.TrailATRPeriod)
     {
      ...
      atr_handle = iATR(Sym(), InpTF, P.TrailATRPeriod);
...
void ManageTrailing(double entry, double &sl, long type, double curr, double risk_per_lot, double reachedR)
  {
   ...
   if(newSL!=sl)
     {
      trade.PositionModify(Sym(), newSL, PositionGetDouble(POSITION_TP));
     }
  }
         ...
         if(lots>0 && SpreadUSD()<=P.MaxSpreadUSD)
           {
            if(AllowedToOpenNow())
              {
               if (SendMarket(false, lots, sl, 0.0)) {      // <-- tp = 0.0
                  g_lastOpenTime    = TimeCurrent();
                  g_lastAddPriceBuy = SymbolInfoDouble(Sym(), SYMBOL_ASK);
                  g_addCount        = 0;
               }
               g_lastOpenTime=TimeCurrent();
               g_lastAddPriceBuy=SymbolInfoDouble(Sym(),SYMBOL_ASK);
            ...
            if(lots>0 && SpreadUSD()<=P.MaxSpreadUSD)
              {
               if(AllowedToOpenNow())
                 {
                  SendMarket(true, lots, sl, 0.0);
                  g_lastOpenTime=TimeCurrent();
                  g_lastAddPriceSell=SymbolInfoDouble(Sym(),SYMBOL_BID);
                  g_addCount=0;
                 }
               g_lastOpenTime = TimeCurrent();
...
int PositionsOnSymbol()
  {
   // Sử dụng PositionSelect trực tiếp với symbol theo MQL5 docs
   if(PositionSelect(Sym()))
     {
      return 1; // Có position cho symbol này
     }
   return 0; // Không có position
  }
...
double CalcLotByRisk(double stop_usd)
  {
   ...
   double tv=0, ts=0;
   SymbolInfoDouble(Sym(), SYMBOL_TRADE_TICK_VALUE, tv);
   SymbolInfoDouble(Sym(), SYMBOL_TRADE_TICK_SIZE, ts);
   if(tv<=0 || ts<=0)
      return 0.0;
      ...
      if(lots>0 && PositionsOnSymbol()<P.MaxOpenPositions && SpreadUSD()<=P.MaxSpreadUSD)
        {
         ...
         if(AllowedToOpenNow())
           {
            ok = SendPending(true, price, lots, sl, 0.0, exp);
            g_lastOpenTime=TimeCurrent();
      ...
      if(lots>0 && PositionsOnSymbol()<P.MaxOpenPositions && SpreadUSD()<=P.MaxSpreadUSD)
        {
         ...
         if(AllowedToOpenNow())
           {
            ok = SendPending(false, price, lots, sl, 0.0, exp);
            g_lastOpenTime=TimeCurrent();
               ...
               if(lots>0 && PositionsOnSymbol()<P.MaxOpenPositions && SpreadUSD()<=P.MaxSpreadUSD)
                 {
                  ...
                  /* print moved into SendMarket on success */
                 }
              }
            state = ST_IDLE;
            return;
...
// Prepare report array based on active symbols
                        ...
                        const MqlTradeResult &result)
{
  ...
  long entry=0;
  if(!HistoryDealGetInteger(deal, DEAL_ENTRY, entry)) return;
  if(entry != DEAL_ENTRY_OUT) return;    // chỉ tính deal đóng lệnh

  string dsym="";                         // tránh trùng với Sym()
  HistoryDealGetString(deal, DEAL_SYMBOL, dsym);
  ...
  // !!! KHÔNG dùng reference (&R) – cập nhật trực tiếp
  gRep[idx].trades++;
  gRep[idx].volume     += vol;
...
// Aggregate from history at the end (backup in case some events missed)
...
// Write CSV into Common/Files
void WriteSymbolReportCSV(const string suffix="")
{
  string t = TimeToString((datetime)TimeLocal(), TIME_DATE|TIME_MINUTES|TIME_SECONDS);
  string fname = StringFormat("MS_Report_%s%s.csv", t, suffix);

  int h = FileOpen(fname, FILE_WRITE|FILE_CSV|FILE_COMMON, ';');
  if(h==INVALID_HANDLE){ Print("Cannot open report: ", fname); return; }

  FileWrite(h,"Symbol","Trades","Wins","Losses","WinRate(%)",
               "Volume","GrossProfit","GrossLoss","Commission","Swap","NetProfit");

  for(int i=0; i<ArraySize(gRep); ++i){
    double wr = gRep[i].trades>0 ? (100.0*gRep[i].wins/gRep[i].trades) : 0.0;
    FileWrite(h,
      gRep[i].sym,
      gRep[i].trades,
      ...
      gRep[i].losses,
      DoubleToString(wr,2),
      DoubleToString(gRep[i].volume,2),
      DoubleToString(gRep[i].grossProfit,2),
      DoubleToString(gRep[i].grossLoss,2),
      DoubleToString(gRep[i].commission,2),
      DoubleToString(gRep[i].swap,2),
      DoubleToString(gRep[i].netProfit,2)
    );
  }
  FileClose(h);
  Print("Report saved: ", fname, " (Common/Files)");
}
...
int OnInit()
  {
   ...
   if(UseMultiSymbol){
      ...
      for(int k=0;k<n;k++){
         ...
         st.sym = s;
         st.last_bar_time = 0;
         st.state_val = ST_IDLE;
         st.bosIsShort=false; st.bosLevel=0; st.bosBarTime=0;
         st.sweepHigh=0; st.sweepLow=0;
         st.g_lastOpenTime=0; st.g_lastAddPriceBuy=0; st.g_lastAddPriceSell=0; st.g_addCount=0;
         st.atr_handle=INVALID_HANDLE; st.last_atr=0.0;
...
void OnTick()
  {
   // Intentionally empty; we drive via OnTimer to support multi-symbol.
  }
...
void OnDeinit(const int reason)
  {
   EventKillTimer();
  // Nếu bạn đã dùng OnTradeTransaction để thu thập thì dòng dưới là “backup” — vẫn nên gọi
...
void LoadStateToGlobals(const SymState &S)
  {
   last_bar_time = S.last_bar_time;
   state         = (StateEnum)S.state_val;
   bosIsShort    = S.bosIsShort;
   bosLevel      = S.bosLevel;
   bosBarTime    = S.bosBarTime;
   sweepHigh     = S.sweepHigh;
   sweepLow      = S.sweepLow;
   g_lastOpenTime     = S.g_lastOpenTime;
   g_lastAddPriceBuy  = S.g_lastAddPriceBuy;
   g_lastAddPriceSell = S.g_lastAddPriceSell;
   g_addCount         = S.g_addCount;
   atr_handle    = S.atr_handle;
   last_atr      = S.last_atr;
  }
void SaveGlobalsToState(SymState &S)
  {
   S.last_bar_time = last_bar_time;
   S.state_val     = (int)state;
   S.bosIsShort    = bosIsShort;
   S.bosLevel      = bosLevel;
   S.bosBarTime    = bosBarTime;
   S.sweepHigh     = sweepHigh;
   S.sweepLow      = sweepLow;
   S.g_lastOpenTime     = g_lastOpenTime;
   S.g_lastAddPriceBuy  = g_lastAddPriceBuy;
   S.g_lastAddPriceSell = g_lastAddPriceSell;
   S.g_addCount         = g_addCount;
   S.atr_handle   = atr_handle;
   S.last_atr     = last_atr;
  }
...
void ProcessOne(const int idx)
  {
   CurrSymbol = gSyms[idx].sym;
   trade.SetExpertMagicNumber(MagicBase + idx);
   LoadStateToGlobals(gSyms[idx]);
   ProcessSingle();
   SaveGlobalsToState(gSyms[idx]);
  }
...
void OnTesterDeinit()
{
  // write unique file per pass to avoid overwrite
  string suf = StringFormat("_opt_%s", IntegerToString((int)GetTickCount()));
  AggregateFromHistory();
  WriteSymbolReportCSV(suf);
}
