//+------------------------------------------------------------------+
//|                PTG BALANCED v3.3.1 (M1 Push–Test–Go)             |
//|  - Safe pending (stops-level & tick-size compliant)               |
//|  - Wick Test + Liquidity Sweep for PTG                            |
//|  - Early cancel pending when structure invalidates                |
//|  - Softer gates; same risk engine as v3.2.1                       |
//|  - FIX: no Point() override; fix undeclared 'i'                   |
//+------------------------------------------------------------------+
#property strict
#property version   "3.31"
#property description "PTG v3.3.1 – bugfix (Point/undeclared), safe pending, wick+sweep PTG"
...
input double    InpPushAvgATRmult       = 0.70;      // AVG ≥ x*ATR
input double    InpPushMaxATRmult       = 0.90;      // OR MAX ≥ y*ATR
input double    InpTestRetrMinPct       = 5.0;       // tightened
input double    InpTestRetrMaxPct       = 90.0;      // tightened
input double    InpWickFracMin          = 0.55;      // min wick fraction of range (test bar)
input bool      InpRequireSweep         = true;      // Test bar sweeps prev swing
...
input double    InpATRMinPips           = 55.0;
...
input double    InpBE_Pips              = 18.0;
input double    InpPartial_Pips         = 20.0;
input double    InpPartial_Percent      = 35.0;
input double    InpTrailStart_Pips      = 22.0;
input double    InpTrailStep_Pips       = 18.0;
input int       InpTimeStopBars         = 10;
input double    InpTimeStopMinProfit    = 5.0;
input int       InpEarlyCutBars         = 2;
input double    InpEarlyCutAdversePips  = 15.0;
...
bool           gBE=false, gPartial=false; int gBarsSinceEntry=0; double gWorstPips=0.0;
...
double ToPrice(double pips){ return pips* gPip; }
double NormalizePrice(double p){ return NormalizeDouble(p,gDigits); }
double SpreadPips(){ double ask=SymbolInfoDouble(_Symbol,SYMBOL_ASK), bid=SymbolInfoDouble(_Symbol,SYMBOL_BID); return (ask-bid)/gPip; }

// --- renamed helpers to avoid built-in clashes
...
double RoundUpToTick(double price){ double ts=TickSizeVal(); return NormalizeDouble(MathCeil(price/ts)*ts,gDigits); }
double RoundDnToTick(double price){ double ts=TickSizeVal(); return NormalizeDouble(MathFloor(price/ts)*ts,gDigits); }
...
bool InBlackout(){
   ...
   if(InpDebug && blk) Print("🌙 BLACKOUT window");
   return blk;
}

double ATRpips(){ double buf[]; if(CopyBuffer(atr_handle,0,0,16,buf)<=0) return 0; ArraySetAsSeries(buf,true); return buf[0]/gPip; }
...
bool RoundNumberNearby(double price){
   ...
   if(InpDebug && hit) Print("⚠️ Round number nearby (",DoubleToString(dMaj/gPip,1)," / ",DoubleToString(dMin/gPip,1)," pips)");
   return hit;
}
bool M5BiasFavor(bool isLong){
   ...
   double slope=(ema[0]-ema[10])/gPip; bool enforce=(fabs(slope)>=InpM5SlopeGatePips);
...
bool BuildPTG(int idx, Setup &s){
   ...
   double avgRangePips = (sumR/InpPushBars)/gPip;
   double maxRangePips = (maxR)/gPip;
   ...
   // Wick test requirement
   double rng = Range(t);
   if(rng <= 0) return false;
   double wickFrac = isLong ? LowerWick(t)/rng : UpperWick(t)/rng;
   if(wickFrac < InpWickFracMin){ if(InpDebug) Print("⚠️ Wick too small: ",DoubleToString(100*wickFrac,1),"%, need ",DoubleToString(100*InpWickFracMin,0),"%"); return false; }

   // Liquidity sweep: test L < prev L (long) / test H > prev H (short)
   if(InpRequireSweep){
      if(isLong){
         if(!(L(t) < L(t+1))) { if(InpDebug) Print("⚠️ No sweep (long)"); return false; }
      }else{
         if(!(H(t) > H(t+1))) { if(InpDebug) Print("⚠️ No sweep (short)"); return false; }
      }
   }
...
bool PlacePending(bool isLong, double price, double sl){
   ...
   if(ok){
      ...
      if(InpDebug) Print("📝 Pending ",(isLong?"BUY STOP ":"SELL STOP "),"@",
                         DoubleToString(price,gDigits)," SL=",DoubleToString(sl,gDigits)," exp=",TimeToString(exp,TIME_SECONDS));
   }else{
...
//===================== POSITION MANAGEMENT ==========================
void ResetPos(){ gHasPosition=false; gPosTicket=0; gPosEntry=0; gPosVolume=0; gBE=false; gPartial=false; gBarsSinceEntry=0; gWorstPips=0.0; }
void AfterOpenSync(){ gHasPosition=true; gPosTicket=(ulong)PositionGetInteger(POSITION_TICKET); gPosEntry=PositionGetDouble(POSITION_PRICE_OPEN); gPosVolume=PositionGetDouble(POSITION_VOLUME); gBarsSinceEntry=0; gBE=false; gPartial=false; gWorstPips=0.0; CancelPendingIfAny(); }
...
void ManagePosition(){
   ...
   double pips=(type==POSITION_TYPE_BUY)? (px-gPosEntry)/gPip : (gPosEntry-px)/gPip;
   gWorstPips = MathMin(gWorstPips, pips);
   gBarsSinceEntry++;

   if(gBarsSinceEntry<=InpEarlyCutBars && pips <= -InpEarlyCutAdversePips){ trade.PositionClose(_Symbol); if(InpDebug) Print("✂️ Early-cut at ",DoubleToString(pips,1),"p after ",gBarsSinceEntry," bars"); return; }
   ...
   if(!gPartial && pips >= InpPartial_Pips){ double vol=PositionGetDouble(POSITION_VOLUME); double closeVol=MathMax(SymbolInfoDouble(_Symbol,SYMBOL_VOLUME_MIN), NormalizeDouble(vol*InpPartial_Percent/100.0,2)); trade.PositionClosePartial(_Symbol,closeVol); gPartial=true; if(InpDebug) Print("💰 Partial ",DoubleToString(closeVol,2)); return; }
   if(!gBE && pips >= InpBE_Pips){ double sl=(type==POSITION_TYPE_BUY)? (gPosEntry+ToPrice(SpreadPips())):(gPosEntry-ToPrice(SpreadPips())); trade.PositionModify(_Symbol,NormalizePrice(sl),0.0); gBE=true; if(InpDebug) Print("🛡️ Move BE"); }
   if(pips>=InpTrailStart_Pips){ double trail=ToPrice(InpTrailStep_Pips); double newSL=(type==POSITION_TYPE_BUY)? (px-trail):(px+trail);
      if( (type==POSITION_TYPE_BUY && newSL>PositionGetDouble(POSITION_SL)) || (type==POSITION_TYPE_SELL && newSL<PositionGetDouble(POSITION_SL)) ) trade.PositionModify(_Symbol,NormalizePrice(newSL),0.0); }
}
...
void ResetDailyIfNeeded(){
   ...
   if(t.day != gLastDay){ gLastDay=t.day; gLossCount=0; gConsecLosses=0; gCooldownUntil=0; if(InpDebug) Print("🔄 New day – reset daily counters"); }
}
...
//=========================== EVENTS =================================
int OnInit(){
   gPip=Pip(); gDigits=(int)SymbolInfoInteger(_Symbol,SYMBOL_DIGITS); gMagic=(ulong)InpMagic; trade.SetExpertMagicNumber((uint)gMagic); ResetPos();
   ...
   gLastBarTime=iTime(_Symbol,PERIOD_CURRENT,0); EventSetTimer(1); Print("PTG v3.3.1 ready. Pip=",DoubleToString(gPip,5)); return INIT_SUCCEEDED;
}
