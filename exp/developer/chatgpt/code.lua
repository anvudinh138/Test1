//+------------------------------------------------------------------+
//|                PTG BALANCED v3.2.1 (M1 Push–Test–Go)             |
//|  - Daily stop auto-reset at new day (and default 999 = off)      |
//|  - Softer squeeze (ATR>=55), softer PUSH (0.70/0.90)             |
//|  - Entry buffer clamped: max(Inp, min(8, spread+2))              |
//|  - M5 bias only when |slope|>=35p                                |
//|  - Round number: major 6p / minor 4p                             |
//+------------------------------------------------------------------+
#property strict
#property version   "3.21"
#property description "PTG v3.2.1 – open gates; daily stop reset; tuned buffers"

#include <Trade/Trade.mqh>

//========================== INPUTS ==================================
input group "=== General ==="
input long      InpMagic                = 321001;
...
input int       InpPushBars             = 1;
input double    InpPushAvgATRmult       = 0.70;     // was 0.75
input double    InpPushMaxATRmult       = 0.90;     // was 0.95
input double    InpTestRetrMinPct       = -35.0;
input double    InpTestRetrMaxPct       = 115.0;
...
input double    InpRoundMajorBufferPips = 6.0;      // was 8
input double    InpRoundMinorGridPips   = 50.0;
input double    InpRoundMinorBufferPips = 4.0;      // was 5

input group "=== Regime Filters ==="
input bool      InpUseSoftSqueeze       = true;
input double    InpATRMinPips           = 55.0;     // was 70
input bool      InpM5Bias               = true;
input int       InpM5EMAPeriod          = 50;
input double    InpM5SlopeGatePips      = 35.0;     // was 60
...
int            gLastDay = -1; // reset daily stop on new day
...
struct Setup { bool valid; bool isLong; double entry; double sl; };
...
bool BuildPTG(int idx, Setup &s){
   ...
   if(!pushOK){
      if(InpDebug) Print("⚠️ PUSH too small: avg=",DoubleToString(avgRangePips,1),"p / max=",DoubleToString(maxRangePips,1),"p  need ",
                         DoubleToString(InpPushAvgATRmult*atr,1)," or ",DoubleToString(InpPushMaxATRmult*atr,1));
      return false;
   }

   double testClose = C(idx+1);
   bool isLong = (C(pushShift) > O(pushShift));
   double retr = RetrPct(ph,pl,testClose,isLong);
...
double CalcPositionSize(double sl){
   ...
   if(ts<=0) ts=SymbolInfoDouble(_Symbol,SYMBOL_POINT); if(tv<=0||dist<=0) return InpFixedLots;
...
void ResetDailyIfNeeded(){
   ...
   if(t.day != gLastDay){
      gLastDay=t.day; gLossCount=0; gConsecLosses=0; gCooldownUntil=0;
      if(InpDebug) Print("🔄 New day – reset daily counters");
   }
}
...
int OnInit(){
   ...
   gLastBarTime=iTime(_Symbol,PERIOD_CURRENT,0); EventSetTimer(1); Print("PTG v3.2.1 ready. Pip=",DoubleToString(gPip,5)); return INIT_SUCCEEDED;
}
...
void OnTimer(){ if(gPendingTicket!=0 && TimeCurrent()>gPendingExpire){ if(InpDebug) Print("⌛ Pending expired – cancel"); CancelPendingIfAny(); } }
void OnTick(){
