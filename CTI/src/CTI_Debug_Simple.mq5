//+------------------------------------------------------------------+
//|                                           CTI_Debug_Simple.mq5 |
//|                     Debug version - Simple BOS/CHoCH detection |
//+------------------------------------------------------------------+
#property copyright "CTI Debug Team"
...
int OnInit()
{
    IndicatorSetString(INDICATOR_SHORTNAME, "CTI Debug Simple");
    ...
    if(DebugMode) Print("[CTI Debug] Initialized successfully");
                ...
                const int &spread[])
{
    ...
    DetectBOSAndCHoCH(rates_total, time, high, low, close);
...
void AddSwingPoint(datetime swingTime, double swingPrice, int swingIndex, SwingType type)
{
    ...
    if(DebugMode) 
    {
        Print("[CTI Debug] Added swing: ", (type == SWING_HIGH ? "HIGH" : "LOW"), 
...
void AnalyzeStructure()
{
    ...
    // Show structure label (offset to avoid overlap)
    if(ShowStructure && structure != STRUCTURE_NONE)
    {
        ...
        // Offset structure labels to avoid overlap
        double offsetPrice = swingPrices[i] + (swingTypes[i] == SWING_HIGH ? 10 : -10) * _Point;
        CreateTextLabel(objName, swingTimes[i], offsetPrice, structureColor, structureText, 10);
    }
        
        // Check for CHoCH immediately after structure change
        CheckForCHoCH(i, structure);
        
        if(DebugMode)
        {
            Print("[CTI Debug] Structure: ", GetStructureText(structure), 
...
void CheckForCHoCH(int swingIndex, StructureType structure)
{
    ...
    if(isCHoCH)
    {
        ...
        // Offset CHoCH labels to avoid overlap with structure labels
        double offsetPrice = swingPrices[swingIndex] + (swingTypes[swingIndex] == SWING_HIGH ? 20 : -20) * _Point;
        CreateTextLabel(objName, swingTimes[swingIndex], offsetPrice, chochColor, text, 12);
        
        if(DebugMode)
        {
            Print("[CTI Debug] CHoCH detected: ", text, " at ", TimeToString(swingTimes[swingIndex]));
        }
    }
}
...
//+------------------------------------------------------------------+
void DetectBOSAndCHoCH(const int total, const datetime &time[], 
                       const double &high[], const double &low[], const double &close[])
{
        ...
        for(int s = swingCount - 1; s >= MathMax(0, swingCount - 5); s--)
        {
            if(swingTypes[s] == SWING_HIGH && swingTimes[s] < currentTime)
            {
                if(currentClose > swingPrices[s])
                {
                    ...
                    {
                        // Offset BOS labels
                        double offsetPrice = currentClose + 30 * _Point;
                        CreateTextLabel(objName, currentTime, offsetPrice, ColorBOS_Bull, "BOS↑", 12);
                        
                        if(DebugMode)
                        {
                            Print("[CTI Debug] Bullish BOS at ", TimeToString(currentTime), 
            ...
            if(swingTypes[s] == SWING_LOW && swingTimes[s] < currentTime)
            {
                if(currentClose < swingPrices[s])
                {
                    ...
                    if(ObjectFind(0, objName) < 0)
                    {
                        // Offset BOS labels
                        double offsetPrice = currentClose - 30 * _Point;
                        CreateTextLabel(objName, currentTime, offsetPrice, ColorBOS_Bear, "BOS↓", 12);
                        
                        if(DebugMode)
                        {
                            Print("[CTI Debug] Bearish BOS at ", TimeToString(currentTime), 
