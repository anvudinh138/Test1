...
input int    PresetCSVId         = 0;   // 0 = bỏ qua; >0 = lấy theo id ở cột đầu CSV
input string PresetCSVFile       = "/XAU_SweepBOS_Presets.csv"; // put in MQL5/Files
...
// Built-in Presets (examples; extendable up to 70+)
bool ApplyPresetBuiltIn(int id){
   // start from safe defaults
   UseInputsAsParams();

   switch(id){
      case 0: return true; // Custom (use inputs)
      case 1: // Baseline-Loose (for sanity check: many trades)
         ...
         P.MaxSpreadUSD=0.80; P.SL_BufferUSD=0.60;
         P.UsePendingRetest=false;
         return true;
      case 2: // RN-Only
         P.UseKillzones=false; P.UseRoundNumber=true; P.RNDelta=0.40; P.UseVSA=false;
         P.K_swing=50; P.N_bos=6; P.M_retest=3; P.EqTol=0.25; P.BOSBufferPoints=1.0;
         P.MaxSpreadUSD=0.60;
         return true;
      case 3: // London Sweep Revert (tight time window)
         P.UseKillzones=true; P.KZ1s=13*60+55; P.KZ1e=14*60+25; // London open (server-agnostic example)
         P.KZ2s=16*60+25; P.KZ2e=16*60+45;    // fix AM
         P.UseRoundNumber=true; P.RNDelta=0.35; P.UseVSA=true; P.L_percentile=150;
         P.K_swing=60; P.N_bos=6; P.M_retest=3; P.EqTol=0.20; P.SL_BufferUSD=0.60;
         return true;
      case 4: // NY Open Rejection
         P.UseKillzones=true; P.KZ3s=19*60+20; P.KZ3e=19*60+50; P.KZ4s=20*60+55; P.KZ4e=21*60+20;
         P.UseRoundNumber=true; P.RNDelta=0.30; P.UseVSA=true; P.L_percentile=120;
         P.K_swing=55; P.N_bos=6; P.M_retest=3; P.EqTol=0.20; P.SL_BufferUSD=0.70;
         return true;
      case 5: // Conservative High-Precision
         P.UseKillzones=true; P.UseRoundNumber=true; P.RNDelta=0.30; P.UseVSA=true; P.L_percentile=180;
         P.K_swing=70; P.N_bos=5; P.M_retest=3; P.EqTol=0.15; P.BOSBufferPoints=2.0;
         P.MaxSpreadUSD=0.50; P.SL_BufferUSD=0.70;
         return true;
      case 6: // Aggressive (more trades)
         P.UseKillzones=false; P.UseRoundNumber=true; P.RNDelta=0.50; P.UseVSA=false;
         P.K_swing=35; P.N_bos=8; P.M_retest=4; P.EqTol=0.35; P.BOSBufferPoints=1.0;
         P.MaxSpreadUSD=0.90; P.SL_BufferUSD=0.55;
         return true;
      case 7: // Asia Range only
         P.UseKillzones=true;
         P.KZ1s=1; P.KZ1e=6*60; P.KZ2s=22*60; P.KZ2e=23*60+59; // sample asia windows (adjust)
         P.UseRoundNumber=true; P.RNDelta=0.40; P.UseVSA=false;
         P.K_swing=50; P.N_bos=6; P.M_retest=5; P.EqTol=0.30; P.SL_BufferUSD=0.50;
         return true;
      case 8: // Pending-retets style
         P.UsePendingRetest=true; P.RetestOffsetUSD=0.07; P.PendingExpirySec=60;
         P.UseKillzones=true; P.UseRoundNumber=true; P.RNDelta=0.35; P.UseVSA=true;
         P.K_swing=55; P.N_bos=6; P.M_retest=3; P.SL_BufferUSD=0.70;
         return true;
      case 9: // No-VSA but strict KZ + RN
         P.UseKillzones=true; P.UseRoundNumber=true; P.UseVSA=false; P.RNDelta=0.30;
         P.K_swing=50; P.N_bos=6; P.M_retest=3; P.SL_BufferUSD=0.60;
         return true;
      case 10: // Ultra-tight (very few trades)
         P.UseKillzones=true; P.UseRoundNumber=true; P.UseVSA=true; P.RNDelta=0.25; P.L_percentile=200;
         P.K_swing=80; P.N_bos=5; P.M_retest=2; P.EqTol=0.12; P.BOSBufferPoints=2.0;
         P.MaxSpreadUSD=0.40; P.SL_BufferUSD=0.75;
         return true;
      default:
         return false;
   }
...
// id,name,EnableLong,EnableShort,K_swing,N_bos,LookbackInternal,M_retest,EqTol,BOSBufferPoints,
...
bool ApplyPresetFromCSVById(int want_id){
   if(want_id<=0) return false;
   int h = OpenCSVHandle(PresetCSVFile,false,false);
   if(h==INVALID_HANDLE) h = OpenCSVHandle(PresetCSVFile,false,true);
   if(h==INVALID_HANDLE) h = OpenCSVHandle(PresetCSVFile,true,false);
   if(h==INVALID_HANDLE) h = OpenCSVHandle(PresetCSVFile,true,true);
   if(h==INVALID_HANDLE){ if(Debug) Print("CSV open failed:",GetLastError()); return false; }

   bool ok=false;
   while(!FileIsEnding(h)){
      int row_id = (int)FileReadInteger(h);     // cột 1: id
      string row_name = StripBOM(FileReadString(h)); // cột 2: name (bỏ qua nếu không cần)
      if(row_id==want_id){
         UseInputsAsParams();
         P.EnableLong      = (bool)FileReadInteger(h);
         P.EnableShort     = (bool)FileReadInteger(h);
         P.K_swing         = (int)FileReadInteger(h);
         P.N_bos           = (int)FileReadInteger(h);
         P.LookbackInternal= (int)FileReadInteger(h);
         P.M_retest        = (int)FileReadInteger(h);
         P.EqTol           = FileReadNumber(h);
         P.BOSBufferPoints = FileReadNumber(h);
         P.UseKillzones    = (bool)FileReadInteger(h);
         P.UseRoundNumber  = (bool)FileReadInteger(h);
         P.UseVSA          = (bool)FileReadInteger(h);
         P.L_percentile    = (int)FileReadInteger(h);
         P.RNDelta         = FileReadNumber(h);
         P.KZ1s=(int)FileReadInteger(h); P.KZ1e=(int)FileReadInteger(h);
         P.KZ2s=(int)FileReadInteger(h); P.KZ2e=(int)FileReadInteger(h);
         P.KZ3s=(int)FileReadInteger(h); P.KZ3e=(int)FileReadInteger(h);
         P.KZ4s=(int)FileReadInteger(h); P.KZ4e=(int)FileReadInteger(h);
         P.RiskPerTradePct = FileReadNumber(h);
         P.SL_BufferUSD    = FileReadNumber(h);
         P.TP1_R           = FileReadNumber(h);
         P.TP2_R           = FileReadNumber(h);
         P.BE_Activate_R   = FileReadNumber(h);
         P.PartialClosePct = FileReadNumber(h);
         P.TimeStopMinutes = (int)FileReadInteger(h);
         P.MinProgressR    = FileReadNumber(h);
         P.MaxSpreadUSD    = FileReadNumber(h);
         P.MaxOpenPositions= (int)FileReadInteger(h);
         P.UsePendingRetest= (bool)FileReadInteger(h);
         P.RetestOffsetUSD = FileReadNumber(h);
         P.PendingExpirySec= (int)FileReadInteger(h);
         ok=true; break;
      }else{
         // bỏ 34 trường còn lại của dòng hiện tại
         for(int i=0;i<34 && !FileIsEnding(h); ++i) FileReadString(h);
      }
   }
   FileClose(h);
   if(Debug) Print("Preset loaded by ID=",want_id," from CSV='",PresetCSVFile,"' ok=",ok);
   return ok;
}
...
bool FiltersPass(int bar){
   if(P.UseRoundNumber && !NearRound(rates[bar].close, P.RNDelta)) { if(Debug) Print("BLOCK RN @", rates[bar].close); return false; }
...
bool SetupParamsFromPreset(){
   ...
   bool ok=false;
   if(UsePreset){
      if(PresetCSVId>0)                ok = ApplyPresetFromCSVById(PresetCSVId);
      else                              ok = ApplyPresetBuiltIn(PresetID);
   } else ok=true;
   if(Debug) Print("Preset applied: ok=",ok," ID=",PresetID," CSVId=",PresetCSVId," CSVName='");
   return ok;
}
