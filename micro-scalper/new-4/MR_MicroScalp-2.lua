...
void ApplyPresetForSymbol(string symbol_name)
{
   ...
   else if(StringFind(symbol_name, "EURUSD") >= 0)
   {
...
{ 
   ...
   if(IN_SYMBOL == "") { G_SYMBOL = _Symbol; }
   else { G_SYMBOL = IN_SYMBOL; }
...
void OnTick()
{
   ...
   if((int)g_last_bar_time % (60 * 60) == 0 && TimeCurrent() - g_last_atr_update > 60) UpdateATRBaseline();

   // --- CÁC BỘ LỌC CHUNG ---
   if(CountEAPositions() >= G_MAX_PYRAMID_ENTRIES) return;
   if(TimeCurrent() - g_last_trade_time < IN_MIN_SECONDS_BETWEEN_TRADES) return;
   
   double spread_ticks = SymbolInfoInteger(G_SYMBOL, SYMBOL_SPREAD) * G_POINT / G_TICK_SIZE;
   if(spread_ticks > G_SPREAD_MAX_TICKS) return;
   ...
   if(IN_ATR_FILTER_ENABLED && G_ATR_BASELINE > 0)
   {
      ...
      if(CopyBuffer(G_ATR_HANDLE, 0, 1, 1, atr_buffer) > 0)
      {
         ...
         if(current_atr > G_ATR_BASELINE * IN_ATR_MAX_MULT) return;
         if(current_atr < G_ATR_BASELINE * IN_ATR_MIN_MULT) return;
      }
   }
