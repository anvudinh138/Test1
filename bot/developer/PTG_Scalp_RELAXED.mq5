//+------------------------------------------------------------------+
//|                        PTG Scalp RELAXED v1.4.0                 |
//|               Relaxed AI Analysis - More Room to Breathe        |
//|                    R&D Version for Better Balance              |
//+------------------------------------------------------------------+
#property strict
#property copyright "Copyright 2024, PTG Trading Strategy"
#property link      "https://github.com/ptg-trading"
#property version   "1.40"
#property description "PTG Scalp RELAXED - AI recommendations with relaxed constraints"
...
input group "=== RELAXED SCALP CONFIG ==="
// Relaxed AI recommendations - allow PTG to breathe
input double   BreakevenPips      = 4.0;               // +X pips => move SL to BE (moderate)
input double   QuickExitPips      = 12.0;              // Balanced TP (not too low)
...
input group "=== RELAXED AI IMPROVEMENTS ==="
// KEY: Better balance between risk filter and signal quantity
input double   MaxRiskPips        = 25.0;              // More relaxed (was 12.0 - too strict!)
input bool     BEOnBarClose       = false;             // Immediate BE (faster protection)
input double   BEOffsetPips       = 0.3;               // Smaller offset (avoid over-engineering)
input bool     EnablePartialTP    = true;              // Keep partial TP
input double   TP1Pips            = 8.0;               // Earlier partial TP
input double   TP1Part            = 0.3;               // Close only 30% (keep more running)
...
input string   BotVersion         = "v1.4.0-RELAXED-BALANCED";
...
int OnInit()
{
   ...
   Print("=== PTG SCALP RELAXED v1.4.0 INITIALIZED ===");
   Print("🧠 RELAXED AI ANALYSIS: Balanced approach for PTG signals");
   Print("🔧 VERSION: ", BotVersion, " | Symbol: ", symbol, " | Pip size: ", pip_size);
   Print("📊 TARGET: Balance signal quantity with quality improvement");
   Print("⚡ RELAXED IMPROVEMENTS:");
   Print("   - MaxRiskPips = ", MaxRiskPips, " (relaxed from 12 to allow more signals)");
   Print("   - QuickExitPips = ", QuickExitPips, " (balanced TP target)");
   Print("   - BreakevenPips = ", BreakevenPips, " (moderate protection)");
   Print("   - PartialTP = ", TP1Part*100, "% @ +", TP1Pips, "p (early partial, keep runners)");
   Print("   - BEOffset = +", BEOffsetPips, "p (minimal engineering)");
   Print("🎯 PHILOSOPHY: Let PTG breathe while adding smart improvements");
...
void OnDeinit(const int reason)
{
   ...
   Print("=== PTG SCALP RELAXED v1.4.0 STOPPED ===");
   Print("Total Signals: ", total_signals, " | Total Trades: ", total_trades);
   Print("🧠 RELAXED AI TEST COMPLETED");
}
...
//+------------------------------------------------------------------+
//| RELAXED: Risk check function - more permissive                 |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| RELAXED: Position management with balanced approach             |
//+------------------------------------------------------------------+
void ManageRelaxedScalpPosition()
{
   string symbol = Symbol();
   ...
   // Early partial TP for quick profit
   if(EnablePartialTP && !partial_tp_triggered && profit_pips >= TP1Pips)
   {
      CheckPartialTP(profit_pips);
      return;
   }

   // Quick breakeven (no waiting for bar close)
   if(!pip_breakeven_activated && profit_pips >= BreakevenPips)
   {
      MoveSLToEntryWithOffset("RELAXED BE at +" + DoubleToString(profit_pips,1) + "p");
      return;
   }
   ...
   {
      ClosePositionAtMarket("RELAXED SCALP at +" + DoubleToString(profit_pips,1) + "p");
      return;
   }
   ...
   if(pip_breakeven_activated && profit_pips >= (last_trail_level + TrailStepPips))
   {
      ...
      {
         MoveSLToPipLevel(new_sl_pips, "RELAXED TRAIL to +" + DoubleToString(new_sl_pips,1) + "p");
...
void CheckPartialTP(double profit_pips)
{
   ...
   if(OrderSend(req, res))
   {
      ...
      if(EnableAlerts) Alert("PTG RELAXED Partial TP +" + DoubleToString(profit_pips,1) + "p");
   }
   else
   {
...
void ClosePositionAtMarket(string reason)
{
   ...
   if(OrderSend(req, res))
   {
      ...
      if(EnableAlerts) Alert("PTG RELAXED " + reason);
   }
   else
   {
...
void MoveSLToEntryWithOffset(string reason)
{
   ...
   if(OrderSend(req, res))
   {
      ...
      Print("🎯 ", reason, " - SL moved to entry+", BEOffsetPips, "p");
      if(EnableAlerts) Alert("PTG RELAXED " + reason);
   }
   else
   {
      Print("❌ BE FAILED: ", res.retcode, " - ", res.comment);
   }
}
...
void PTG_MainLogic()
{
   ...
   if(push_up || push_down)
   {
      ...
      if(EnableDebugLogs && (total_signals % 100 == 0))
         Print("🔥 RELAXED PUSH #", total_signals, " ", push_up ? "UP" : "DOWN");
   }
   ...
   if(push_bar_index >= 1 && push_bar_index <= TestBars)
   {
      ...
      if(test_long || test_short)
      {
         ...
         if(test_long)
         {
            ...
            // RELAXED: Check risk before executing
            if(RiskOk(entry_level, sl_level))
               ExecuteRelaxedScalpTrade(ORDER_TYPE_BUY_STOP, entry_level, sl_level, tp_level, "PTG Relaxed Long");
         }
         else
         {
            ...
            // RELAXED: Check risk before executing
            if(RiskOk(entry_level, sl_level))
               ExecuteRelaxedScalpTrade(ORDER_TYPE_SELL_STOP, entry_level, sl_level, tp_level, "PTG Relaxed Short");
         }
         wait_test = false;
      }
   }
...
//+------------------------------------------------------------------+
void ExecuteRelaxedScalpTrade(ENUM_ORDER_TYPE order_type, double entry_price, double sl_price, double tp_price, string comment)
{
   string symbol = Symbol();
   total_trades++;
   ...
   req.comment = comment;
   req.magic   = 22222; // Relaxed Scalp magic
   req.deviation = 10;

   if(OrderSend(req, res))
   {
      string alert_msg = "PTG RELAXED " + (order_type==ORDER_TYPE_BUY_STOP?"LONG":"SHORT") + 
      ...
      Print("✅ RELAXED SCALP: ", (order_type==ORDER_TYPE_BUY_STOP?"LONG":"SHORT"),
   ...
   {
      Print("❌ RELAXED TRADE #", total_trades, " FAILED: ", res.retcode, " - ", res.comment);
      total_trades--;
      ResetPositionVariables();
   }
}
   ...
   if(trans.type == TRADE_TRANSACTION_DEAL_ADD)
   {
      if(trans.deal_type == DEAL_TYPE_BUY || trans.deal_type == DEAL_TYPE_SELL)
      {
         ...
         if(!position_active && trans.position > 0)
         {
            ...
            Print(msg); 
            if(EnableAlerts) Alert("PTG RELAXED ENTRY " + (trans.deal_type == DEAL_TYPE_BUY ? "LONG" : "SHORT"));
         }
         else if(position_active)
         {
            ...
            string msg = StringFormat("💰 RELAXED EXIT %s: %s%.1f pips", exit_type, profit_sign, profit_pips);
            Print(msg); 
            if(EnableAlerts) Alert("PTG RELAXED " + msg);
