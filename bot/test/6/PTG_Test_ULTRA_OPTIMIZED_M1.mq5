//+------------------------------------------------------------------+
//|                           PTG_Test_ULTRA_OPTIMIZED_M1.mq5       |
//|                                    ULTRA-OPTIMIZED FOR M1       |
//|                              Target: Efficiency <2.0, Win >65%  |
//+------------------------------------------------------------------+
#property copyright "PTG Test Suite - Ultra Optimized"
#property version   "1.0U"
#property description "ULTRA-OPTIMIZED M1: Fixed P&L + Disabled broken filters"

//--- ULTRA-OPTIMIZED PARAMETERS FOR M1
...
input group "=== ULTRA-OPTIMIZED PUSH PARAMETERS ==="
input double   PushRangePercent   = 0.60;              // 60% - Stricter than optimized (was 0.55)
input double   ClosePercent       = 0.70;              // 70% - Much stricter (was 0.65)
input double   OppWickPercent     = 0.40;              // 40% - Stricter wick filter (was 0.45)
input double   VolHighMultiplier  = 1.5;               // 1.5x - Higher volume requirement (was 1.4)
input double   MinRangePips       = 20.0;              // 20 pips minimum (was 15.0)

input group "=== SIMPLIFIED TEST PARAMETERS ==="
input int      TestBars           = 10;                // Longer test window (was 6)
input int      PendingTimeout     = 12;                // Longer timeout (was 8)
input double   PullbackMax        = 0.80;              // More pullback allowed (was 0.65)
input double   VolLowMultiplier   = 1.0;               // Standard test volume (was 0.9)
input double   TestRangeMax       = 0.80;              // Relaxed test range (was 0.75)

input group "=== FIXED SIGNAL QUALITY FILTERS ==="
input bool     UseConsecutiveFilter = true;           // Keep consecutive filter
input int      MinBarsBetweenSignals = 5;             // More spacing (was 3)
input bool     UseVolumeSpike      = false;           // DISABLED - was broken!
input double   VolumeSpikeMultiplier = 1.0;           // Disabled (was 1.2)

input group "=== RISK MANAGEMENT ==="
input double   EntryBufferPips    = 0.5;               // Standard entry buffer
input double   SLBufferPips       = 0.5;               // Standard SL buffer
input double   TPMultiplier       = 2.0;               // Standard R:R (was 2.1)
input bool     UseTrailingStop    = true;              // Enable trailing stop
input double   TrailingStopPips   = 15.0;             // Standard trailing (was 12.0)
input double   RiskPercent        = 0.5;               // Conservative risk
input double   MaxSpreadPips      = 45.0;              // Moderate spread filter
...
input int      MinBarsBetweenTrades   = 3;             // Standard trade frequency
...
int OnInit()
{
    ...
    Print("=== PTG ULTRA-OPTIMIZED M1: TARGET EFFICIENCY <2.0 ===");
    Print("Symbol: ", symbol, " | Pip size: ", pip_size);
    Print("ULTRA-OPTIMIZED PARAMETERS:");
    Print("Push Range: ", PushRangePercent*100, "% (vs 55% optimized)");
    Print("Close Position: ", ClosePercent*100, "% (vs 65% optimized)");
    Print("Volume Multiplier: ", VolHighMultiplier, "x (vs 1.4x optimized)");
    Print("Min Range: ", MinRangePips, " pips (vs 15p optimized)");
    Print("Volume Spike: ", UseVolumeSpike ? "ENABLED" : "DISABLED (FIXED!)");
    Print("TARGET: <10 signals/day, <5 trades/day, >65% win rate, efficiency <2.0");
...
void OnDeinit(const int reason)
{
    ...
    Print("=== ULTRA-OPTIMIZED M1 COMPLETED ===");
...
void ManageTrailingStop()
{
    ...
    for(int i = 0; i < positions; i++)
    {
        ...
        if(PositionGetInteger(POSITION_MAGIC) != 77777) continue; // Ultra-optimized magic
...
//+------------------------------------------------------------------+
//| Ultra-Optimized PTG Logic for M1 Efficiency                     |
//+------------------------------------------------------------------+
void PTG_UltraOptimizedLogic()
{
    string symbol = Symbol();
    ...
    // ULTRA-STRICT conditions - removed broken volume spike filter
    ...
    bool push_up = up_trend && big_range && high_volume && consecutive_ok &&
                   (close_pos_hi >= ClosePercent) && (up_wick <= OppWickPercent);
    bool push_down = down_trend && big_range && high_volume && consecutive_ok &&
    ...
    if(push_up || push_down)
    {
        ...
        if(EnableDebugLogs)
        {
            string msg = StringFormat("🔥 ULTRA PUSH #%d %s | Range: %.1fp | Vol: %.0f/%.0f (%.1fx) | Close: %.1f%% | Wick: %.1f%%", 
    ...
    if(wait_test)
    {
        ...
        if(push_bar_index >= 1 && push_bar_index <= TestBars)
        {
            ...
            bool small_range = (range <= max_range * TestRangeMax);
            ...
            if(test_long || test_short)
            {
                ...
                if(test_long)
                {
                    ...
                    if(EnableDebugLogs)
                        Print("🚀 ULTRA LONG #", total_signals, " → TRADE #", total_trades + 1);
                    
                    ExecuteTrade(ORDER_TYPE_BUY_STOP, entry_level, sl_level, tp_level, "PTG Ultra Long");
                }
                else if(test_short)
                {
                    ...
                    if(EnableDebugLogs)
                        Print("🔻 ULTRA SHORT #", total_signals, " → TRADE #", total_trades + 1);
                    
                    ExecuteTrade(ORDER_TYPE_SELL_STOP, entry_level, sl_level, tp_level, "PTG Ultra Short");
                }
...
//+------------------------------------------------------------------+
//| Execute trade with ultra-optimized risk management              |
//+------------------------------------------------------------------+
void ExecuteTrade(ENUM_ORDER_TYPE order_type, double entry_price, double sl_price, double tp_price, string comment)
{
    ...
    request.magic = 77777; // Ultra-optimized magic number
    request.deviation = 10;
    
    if(OrderSend(request, result))
    {
        string msg = StringFormat("✅ ULTRA TRADE #%d: %s %.2f lots | Risk: %.1fp | R:R %.1fx | Efficiency: %.2f",
                                 total_trades,
    ...
    {
        Print("❌ ULTRA TRADE #", total_trades, " FAILED: ", result.retcode, " - ", result.comment);
        total_trades--;
    }
}

//+------------------------------------------------------------------+
//| Trade transaction handler - FIXED P&L CALCULATION              |
//+------------------------------------------------------------------+
                       ...
                       const MqlTradeResult& result)
{
    ...
    static ENUM_POSITION_TYPE last_position_type = POSITION_TYPE_BUY;
    ...
    if(trans.type == TRADE_TRANSACTION_DEAL_ADD)
    {
        if(trans.deal_type == DEAL_TYPE_BUY || trans.deal_type == DEAL_TYPE_SELL)
        {
            if(!is_position_open)
            {
                ...
                last_position_type = (trans.deal_type == DEAL_TYPE_BUY) ? POSITION_TYPE_BUY : POSITION_TYPE_SELL;
                
                string msg = StringFormat("🎯 ULTRA ENTRY: %s %.2f lots at %.5f | Current Efficiency: %.2f",
            ...
            {
                // This is an exit - FIXED CALCULATION
                ...
                if(last_position_type == POSITION_TYPE_BUY)
                {
                    ...
                    profit_pips = (exit_price - last_entry_price) / pip_size;
                }
                else
                {
                ...
                string msg = StringFormat("💰 ULTRA EXIT %s: %s%.1f pips | Entry: %.5f | Exit: %.5f | Efficiency: %.2f",
